...
function imageUpdatesize($picname,$maxx=100,$maxy=100,$pre="s_"){
    ...
    switch($info[2]){
        case 1://gif
            $im1=imagecreatefromgif($picname);
            break;
        case 2://jpg
            $im1=imagecreatefromjpeg($picname);
            break;
        case 3://png
            $im1=imagecreatefrompng($picname);
            break;
//        default:
//            die("图像类型错误");
//            break;
    ...
    //执行等比缩放
    imagecopyresampled($nim,$im1,0,0,0,0,$nw,$nh,$w,$h);
...
//生成网址的二维码 返回图片地址
function haQrCode($token,$url,$size = 8,$patchs='',$fuid){
    $name = date('Y/m/d/', NOW_TIME);
    $md5 = md5($token);
    $new= substr($token,5);
    $username = D('UserS')->find($new);
    //如果有定义路径
    if($patchs){
        $patch =BASE_PATH.'/attachs/'.$patchs.'/'.$name;
    }else{
        $patch =BASE_PATH.'/attachs/'. 'weixin/'.$name;
        $patch1 =BASE_PATH.'/attachs/'. 'weixin/'.$name;
    }
    $patch =BASE_PATH.'/attachs/'. 'weixin/'.$name;
    mkdir($patch1,0755,true);
    if(!file_exists($patch)){
        mkdir($patch,0755,true);
    }
    if(!$patchs){
    ...
    //设置保存图片
    $url = BASE_PATH.$data[1][lp];
    //二维码
    //开始添加水印操作
    // 图片格式png
    $im = imagecreatefrompng($url);
    $width_xx = (int)substr($data[0]['left'],0,strrpos($data[0]['left'],'p'))*2;//取type=qr 的 left值
    $height_yy = (int)substr($data[0]['top'],0,strrpos($data[0]['top'],'p'))*2;//取type=qr 的 left值
    $width_xxx = (int)substr($data[0]['width'],0,strrpos($data[0]['width'],'p'))*2;
    // 调用方法等比例缩放
    $myimg = imageUpdatesize(BASE_PATH.$file,$width_xxx,$width_xxx,ss_);
    file_put_contents($patch."logo".$fuid.".png", file_get_contents($myimg));
    // 图片格式png
    $logo1 = @imagecreatefrompng($patch."logo".$fuid.".png");
    //拷贝新二维码
    imagecopy($im, $logo1, $width_xx ,$height_yy, 0 ,00, $width_xxx,$width_xxx);
    //保存图片
    imagepng($im,$patch."hb".$fuid."-1.png");
    imagedestroy($im);
    imagedestroy($logo1);
    //默认背景图片
    $url1 = $patch."hb".$fuid."-1.png";
    //头像
    $Users = D('Users')->where(array('user_id'=>$fuid))->find();
    if($Users['poster']){
        $url1a = BASE_PATH.$Users['poster'];
    }else{
        if($Users['face']){
            $url1a = __HOST__.$Users['face'];
        }else{
            $url1a = BASE_PATH.'/attachs/default.jpg';
//            $url1a = "http://192.168.1.104/attachs/default.jpg";
//            $url1a = "http://thirdwx.qlogo.cn/mmopen/vi_32/Q0j4TwGTfTKgDoLoROqFw8LmDSOt90icvNAic8uuTUquR6SaRknKCHFYc5OKibYwDJ13dicBd8PcibIPy7HA9QmnibJA/132";
//            $url1a ="https://images2015.cnblogs.com/blog/1021230/201704/1021230-20170427161154444-2100426627.png";
//            $url1a = "http://pic.qiantucdn.com/58pic/28/23/63/72I58PICdpZ_1024.jpg!qtwebp324";
        }
    }
    if(strstr($url1a,'//'))
    {
        $aa =file_get_contents($url1a);
        file_put_contents($patch."load".$fuid.".jpg", $aa);
        $url1a = $patch."load".$fuid.".jpg";
        $url1a = BASE_PATH.'/attachs/default.jpg';
    }
    //开始添加水印操作
    // 图片格式jpg
    $im1 = @imagecreatefrompng($url1);
    $width_xx1 = (int)substr($data[1]['left'],0,strrpos($data[1]['left'],'p'))*2;//取type=qr 的 left值
    $height_yy1 = (int)substr($data[1]['top'],0,strrpos($data[1]['top'],'p'))*2;//取type=qr 的 left值
    $width_xxx1 = (int)substr($data[1]['width'],0,strrpos($data[1]['width'],'p'))*2;
    $myimg1 = imageUpdatesize($url1a,$width_xxx1,$width_xxx1,ss_);
    file_put_contents($patch."pic".$fuid.".png", file_get_contents($myimg1));
    $logo2 = @imagecreatefromjpeg($patch."pic".$fuid.".png");
    //拷贝新二维码
    imagecopy($im1, $logo2, $width_xx1 ,$height_yy1, 0 ,00, $width_xxx1,$width_xxx1);
    //保存图片
    imagejpeg($im1,$patch."hb".$fuid."-2.jpg");
    imagedestroy($logo2);
    imagedestroy($im1);
    $url2 = $patch."hb".$fuid."-2.jpg";
    //文字
    $font = BASE_PATH.'/static/fonts/font20353/2.otf';//字体,字体文件需保存到相应文件夹下
    $myurl = BASE_PATH.'/attachs/'. 'weixin/'.$name."hb".$fuid."-2.jpg";
    $img = @imagecreatefromstring(file_get_contents($myurl));
    //取消
    $color3 = hex2rgb($data[2]['color']);
    //$black = imagecolorallocate($img, 	78, 238, 148);
    //$black = imagecolorallocate($img, $color3['red'], $color3['green'], $color3['blue']);//字体颜色 RGB
//    $fontsize2 = $data[2]['size'];
//    $fontsize3 = str_replace("px","",$fontsize2);
    //字体颜色
//    $fontSize = $fontsize3;   //字体大小
    $circleSize = 0; //旋转角度
    $width_xx2 = (int)substr($data[2]['left'],0,strrpos($data[2]['left'],'p'))*2;//取type=qr 的 left值
    $height_yy2 = (int)substr($data[2]['top'],0,strrpos($data[2]['top'],'p'))*2;//取type=qr 的 left值
    $width_xxx2 = (int)substr($data[2]['width'],0,strrpos($data[2]['width'],'p'))*2;
    $height_xxx3 = (int)substr($data[2]['height'],0,strrpos($data[2]['height'],'p'))*2;
    //img由图像创建函数返回的图像资源
    //size要使用的水印的字体大小
    ...
    @imagefttext($img, 22, $circleSize, $width_xx2, $height_yy2+$height_xxx3, 0, $font, $username['nickname']);
    //颜色 取消
    //imagefttext($img, $fontsize3, $circleSize, $width_xx2, $height_yy2+$height_xxx3, $black, $font, $username['nickname']);
    $myurl1 = '/attachs/'. 'weixin/'.$name."hb".$fuid."-3.png";
    //image 图像以 filename 为文件名创建一个 JPEG 图像
    imagepng($img,$patch."hb".$fuid."-3.png");
    imagedestroy($img);
    return $myurl1;
}
...
//
//生成网址的二维码 返回图片地址
function haQrCodes($token,$url,$size = 8,$patchs='',$fuid){
    $name = date('Y/m/d/', NOW_TIME);
    $md5 = md5($token);
    $new= substr($token,5);
    $username = D('UserS')->find($new);
...
//
//生成网址的二维码 返回图片地址
function haQrCodess($token,$url,$size = 8,$patchs='',$fuid){
    $name = date('Y/m/d/', NOW_TIME);
    $md5 = md5($token);
    $new= substr($token,5);
    $username = D('UserS')->find($new);
...
//
//生成网址的二维码 返回图片地址
function haQrCodesss($token,$url,$size = 8,$patchs='',$fuid){
    $name = date('Y/m/d/', NOW_TIME);
    $md5 = md5($token);
    $new= substr($token,5);
    $username = D('UserS')->find($new);
...
//去掉权限
function BACELL($url = '', $vars = '', $title = '', $mini = "", $class = "", $width = '', $height = '') {
    $url = U($url, $vars);
    //权限判断 暂时忽略，后面补充
    $m = $c = $h = $w = '';
    if (!empty($mini)) {
...
/**
 * 刷新商品库存, 如果商品有设置规格库存, 则商品总库存 等于 所有规格库存相加=====便利店
 * @param type $goods_id  商品id
 */
function refresh_store_stock($goods_id){
    $count = M("StoreSpecGoodsPrice")->where("product_id = $goods_id")->count();
    if($count == 0) return false; // 没有使用规格方式 没必要更改总库存

    $store_count = M("StoreSpecGoodsPrice")->where("product_id = $goods_id")->sum('store_count');
}
 ...
 *根据订单id来刷新相应的多属性的规格
 */
function refresh_spec_exchange($goods_id,$key,$num){
    $spt=D('ExchangeSpecGoodsPrice')->where("`key`='{$key}' and `goods_id`='{$goods_id}'")->find();
    if(empty($spt)) return false;
    // 没有使用规格方式
    //库存操作库存
    $data['store_count'] = $spt['store_count'] + $num;
    D('ExchangeSpecGoodsPrice')->where("`key`='{$key}' and `goods_id`='{$goods_id}'")->save($data);;
}
 ...
 * 判断规格的库存是否还充足=========>会员专享礼品
 */
function is_spec_exchange($goods_id,$key,$num){
    $spt=D('ExchangeSpecGoodsPrice')->where("`key`='{$key}' and `goods_id`='{$goods_id}'")->find();
    if(empty($spt)) return true; // 没有使用规格 也就不需要判断了
...
/**
 *根据订单id来刷新相应的多属性的规格====积分商城
 */
function refresh_jifen_stock($goods_id,$key,$num){
    $spt=D('Integralgoodsspecprice')->where("`key`='{$key}' and `goods_id`='{$goods_id}'")->find();
    if(empty($spt)) return false;
    // 没有使用规格方式
    //库存操作库存
    $data['store_count'] = $spt['store_count'] + $num;
    D('Integralgoodsspecprice')->where("`key`='{$key}' and `goods_id`='{$goods_id}'")->save($data);;
}
 ...
 * 判断规格的库存是否还充足=======积分商城
 */
function is_jifen_stock($goods_id,$key,$num){
    $spt=D('Integralgoodsspecprice')->where("`key`='{$key}' and `goods_id`='{$goods_id}'")->find();
    if(empty($spt)) return true; // 没有使用规格 也就不需要判断了
 ...
 *获取单个规格的库存=======会员专享礼品
 */
function get_one_spec_exchange($goods_id,$key){
    $spt=D('Integralgoodsspecprice')->where("`key`='{$key}' and `goods_id`='{$goods_id}'")->find();
    if(empty($spt)) return 0; // 没有使用规格 也就不需要判断了
 ...
 * 判断规格的库存是否还充足========积分商城
 */
function is_spec_stock_jifen($goods_id,$key,$num){
    $spt=D('Integralgoodsspecprice')->where("`key`='{$key}' and `goods_id`='{$goods_id}'")->find();
    if(empty($spt)) return true; // 没有使用规格 也就不需要判断了
 ...
 *获取单个规格的库存================积分商城
 */
function get_one_spec_stock_jifen($goods_id,$key){
    $spt=D('Integralgoodsspecprice')->where("`key`='{$key}' and `goods_id`='{$goods_id}'")->find();
    if(empty($spt)) return 0; // 没有使用规格 也就不需要判断了
...
function getChannelName($channel_id){  
  ...
  //数组去掉重复，按照键值 
function getChannelCate($channel_id){  
    $typename = M('LifeType')->where(array('type_id'=>$channel_id))->getField('typename');
   return $typename;
}
//根据类型计算积分	
...
<div align='right' valign='top'><span style='position:relative;right:20px;'>" . $t . "</span>
        ...
        $info = D('Usersign')->field('user_id')->where("last_time = " . $t2 . "  AND user_id = " . $user_id . "")->find();
...
//时间比较
function get_curr_time_section($times,$dingtime)
{
    $sptime = explode('-', $times);
    $checkDayStr = date('Y-m-d ',time());
    $timeBegin1 = strtotime($checkDayStr.$sptime[0].":00");
    $timeEnd1 = strtotime($checkDayStr.$sptime[1].":00");
    $curr_time = strtotime($checkDayStr.$dingtime.":00");
...
//返回级别
function levelpsy($level){
    $obj = D('DeliveryLevel')->where('id='.$level)->find();
    return $obj['title'];
}

// //条件查询
// function  join_where($number,$symbol){
//     if($symbol=='='){
//         return array('eq',$number);
//     }
//     if($symbol=='<'){
//         return array('lt',$number);
//     }
//     if($symbol=='>'){
//         return array('gt',$number);
//     }
//     if($symbol=='-'){
//         list($value1,$value2)=explode('-',$number);
//         return array('between',array($value1,$value2));
//     }
//     return array('gt',0);
// }

//计算外卖配送费
if( !function_exists("compare_price")){
    function compare_price($juli ,$type){
        $config = D('Setting')->fetchAll();
            //专送
        ...
        if($type==1){
            ...
            }else if($juli > $zone_km && $juli <= $ztwo_km){
                $val['logistics'] = $ztwo_money;

            }else if($juli > $ztwo_km && $juli <= $zthree_km ){
                $val['logistics'] = $zthree_money;

            }else if($juli > $zthree_km && $juli <=$zfour_km){
                $val['logistics'] =$zfour_money;

            }else if($juli > $zfour_km && $juli <= $zfive_km){
                $val['logistics'] = $zfive_money;
            }
            return $val['logistics'];
        }
        ...
        if($type==2){
            ...
            }else if($juli > $zdone_km && $juli <= $zdtwo_km){
                $val['zdlogistics'] = $zdtwo_money;

            }else if($juli> $zdtwo_km && $juli <= $zdthree_km){
                $val['zdlogistics'] = $zdthree_money;

            }else if($juli> $zdthree_km && $juli <= $zdfour_km){
                $val['zdlogistics'] = $zdfour_money;

            }else if($juli > $zdfour_km && $juli <= $zdfive_km){
                $val['zdlogistics'] = $zdfive_money;
            }
            return  $val['zdlogistics'];
        }
...
//获取商家销售总订单数
function getShopOrderCount($cate_id,$shop_id,$create_time){
    $where['status']=8;
    //商城
    if($cate_id==99){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('OrderGoods')->where($where)->count();
    ...
    if($cate_id==100){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('EleOrder')->where($where)->count();
        $count=isset($count)?$count:0;
        return $count;
    }
    //菜市场
    if($cate_id==101){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('MarketOrder')->where($where)->count();
        $count=isset($count)?$count:0;
        return $count;
    }
    //便利店
    if($cate_id==102){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('StoreOrder')->where($where)->count();
        $count=isset($count)?$count:0;
        return $count;
    }
    //KTV
    if($cate_id==103){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('KtvOrder')->where($where)->count();
        $count=isset($count)?$count:0;
        return $count;
    }
    //酒店
    if($cate_id==104){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('HotelOrder')->where($where)->count();
        $count=isset($count)?$count:0;
        return $count;
    }
    //教育
    if($cate_id==105){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('EduOrder')->where($where)->count();
        $count=isset($count)?$count:0;
        return $count;
    }
    //家政
    if($cate_id==106){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('AppointOrder')->where($where)->count();
        $count=isset($count)?$count:0;
        return $count;
    }
    ...
    if($cate_id==107){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('FarmOrder')->where($where)->count();
        $count=isset($count)?$count:0;
        return $count;
    }
...
//获取商家销售总订单金额
function getShopOrderMoney($cate_id,$shop_id,$create_time){
    $where['status']=8;
    //商城
    if($cate_id==99){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('OrderGoods')->where($where)->sum('price');
        $count=isset($count)?$count:0;
        return $count;
    }
    //外卖
    if($cate_id==100){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('EleOrder')->where($where)->sum('price');
        $count=isset($count)?$count:0;
        return $count;
    }
    //菜市场
    if($cate_id==101){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('MarketOrder')->where($where)->sum('price');
        $count=isset($count)?$count:0;
        return $count;
    }
    //便利店
    if($cate_id==102){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('StoreOrder')->where($where)->sum('price');
        $count=isset($count)?$count:0;
        return $count;
    }
    //KTV
    if($cate_id==103){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('KtvOrder')->where($where)->sum('price');
        $count=isset($count)?$count:0;
        return $count;
    }
    //酒店
    if($cate_id==104){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('HotelOrder')->where($where)->sum('price');
        $count=isset($count)?$count:0;
        return $count;
    }
    //教育
    if($cate_id==105){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('EduOrder')->where($where)->sum('price');
        $count=isset($count)?$count:0;
        return $count;
    }
    //家政
    if($cate_id==106){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('AppointOrder')->where($where)->sum('price');
        $count=isset($count)?$count:0;
        return $count;
    }
    ...
    if($cate_id==107){
        $where['shop_id']=$shop_id;
        $where['create_time']=$create_time;
        $count=M('FarmOrder')->where($where)->sum('price');
        $count=isset($count)?$count:0;
        return $count;
    }
...
//获取商家每月销售订单和销售金额
function getShopEveryMonthData($cate_id,$shop_id){
    $where['status']=8;
    //商城
    if($cate_id==99){
        $where['shop_id']=$shop_id;
        $list=M('OrderGoods')
            ->where($where)
            ->field("FROM_UNIXTIME(create_time,'%Y-%m') months,count(1) order_count,sum(price) total_price")
            ->group('months')
            ->select();
        return $list;
    }
	...
	//根据板块id返回不同的订单表
function  getShopOrderTable($cate_id){
    //商城
    if($cate_id==99){
        return M('Order');
    }
...
//阿里云OSS存储
 function alioss(){
     $upinfo = M("uploadset")->where(['type'=>'Aliyun'])->find();
     $arr=json_decode($upinfo['para'],true);

    Vendor('OSS.autoload');
    $oss=new \OSS\OssClient($arr['accessKey'],$arr['secrectKey'],$arr['domain']);
    return $oss;
}


//根据order_id返回相同的字段
function  getShopOrderData($cate_id,$shop_id){
    $where['status']=8;
    $where['is_profit']=0;
    $where['shop_id']=$shop_id;
    //商城
    if($cate_id==99){
        $list=M('Order')->where($where)->field('order_id,user_id,need_pay as money')->select();
        return $list;
    }
    //外卖
    if($cate_id==100){
        return M('EleOrder')->where($where)->field('order_id,user_id,need_pay as money')->select();
    }
    //菜市场
    if($cate_id==101){
        return M('MarketOrder')->where($where)->field('order_id,user_id,need_pay as money')->select();
    }
    //便利店
    if($cate_id==102){
        return M('StoreOrder')->where($where)->field('order_id,user_id,need_pay as money')->select();
    }
    //KTV
    if($cate_id==103){
        return M('KtvOrder')->where($where)->field('order_id,user_id,price as money')->select();
    }
    //酒店
    if($cate_id==104){
        return M('HotelOrder')->where($where)->field('order_id,user_id,price as money')->select();
    }
    //教育
    if($cate_id==105){
        return M('EduOrder')->where($where)->field('order_id,user_id,need_pay as money')->select();
    }
    //家政
    if($cate_id==106){
        return M('AppointOrder')->where($where)->field('order_id,user_id,need_pay as money')->select();
    }

    //农家乐
    if($cate_id==107){
        return M('FarmOrder')->where($where)->field('order_id,user_id,amount as money')->select();
    }
    return [];
}
...
//计算配送费
function Distributionfee($lat,$lng,$shop_id,$type){
         $config = D('Setting')->fetchAll();
        //等于外卖
        if($type==1){
            ...
            //计算专送配送费
            if($distance <= $zone_km){
                $logistics= $zone_money;

            }else if($distance > $zone_km && $distance <= $ztwo_km){
                $logistics = $ztwo_money;

            }else if($distance > $ztwo_km && $distance <= $zthree_km ){
                $logistics = $zthree_money;

            }else if($distance > $zthree_km && $distance <=$zfour_km){
                $logistics =$zfour_money;

            }else if($distance > $zfour_km && $distance <= $zfive_km){
                $logistics = $zfive_money;
            }
            ...
            //计算直达配送费
            if($distance <= $zdone_km){
               $zdlogistics = $zdone_money;

            }else if($distance > $zdone_km && $distance <= $zdtwo_km){
                $zdlogistics = $zdtwo_money;

            }else if($distance> $zdtwo_km && $distance <= $zdthree_km){
                $zdlogistics = $zdthree_money;

            }else if($distance> $zdthree_km && $distance <= $zdfour_km){
                $zdlogistics = $zdfour_money;

            }else if($distance> $zdfour_km && $distance <= $zdfive_km){
                $zdlogistics = $zdfive_money;
            }
            //菜市场
        }elseif($type==2) {
            $mshop = D('Market')->find($shop_id);
            $distance = getDistance($lat, $lng, $mshop['lat'], $mshop['lng']);
            //专送
            $zone_km = $config['freight']['market_distance'];//最低公里数
            $zone_money = $config['freight']['market_money'];//最低起步价
            $zone_jia = $config['freight']['market_jiaja'];//超过最低起步价，没公里加价
            //计算专送配送费
            if ($distance <= $zone_km) {
                $logistics = $zone_money;

            } else if ($distance > $zone_km) {
                $logistics = $zone_money + (($distance - $zone_km) * $zone_jia);
            }

            //直达
            $zdone_km = $config['freight']['zdmarket_distance'];
            $zdone_money = $config['freight']['zdmarket_money'];
            $zdone_jia = $config['freight']['zdmarket_jiaja'];
            //计算直达配送费
            if ($distance <= $zdone_km) {
                $zdlogistics = $zdone_money;

            } else if ($distance > $zdone_km) {
                $zdlogistics = $zdone_money + (($distance - $zdone_km) * $zdone_jia);

            }
            //便利店
        }elseif($type==3){
            $sshop = D('Store')->find($shop_id);
            $distance = getDistance($lat, $lng, $sshop['lat'], $sshop['lng']);
            //专送
            $zone_km=$config['freight']['store_distance'];//最低公里数
            $zone_money=$config['freight']['store_money'];//最低起步价
            $zone_jia=$config['freight']['store_jiaja'];//超过最低起步价，没公里加价
            //计算专送配送费
            if($distance <= $zone_km){
                $logistics = $zone_money;
            ...
            //直达
            $zdone_km=$config['freight']['zdstore_distance'];
            $zdone_money=$config['freight']['zdstore_money'];
            $zdone_jia=$config['freight']['zdstore_jiaja'];
            //计算直达配送费
            if($distance <= $zdone_km){
                $zdlogistics = $zdone_money;
        ...
        //查询是否是晚上单
        $bg_time=$config['delivery']['times'];//开始时间
        $end_time=$config['delivery']['end_times'];//结束时间
        $now_time=time();//获取当前时间
        $timeBetween=[$bg_time,$end_time];
        $panduna=getTime($now_time,$timeBetween);
        if($panduna==1){
            //知道了现在的配送费
            $jia=$logistics+($logistics/2);
            $zjia=$zdlogistics+($zdlogistics/2);
            $pei=array(
                'logistics'=>$jia,
                'zdlogistics'=>$zjia,
            );
