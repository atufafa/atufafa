...
function haQrCode($token,$url,$size = 8,$patchs='',$fuid){ //生成网址的二维码 返回图片地址
    ...
    // print_r($file);
    //默认背景图片
    $url = BASE_PATH.'/attachs/2018/12/13/5c1239f8a5395.png';
    //取setting表中k=poster得的值
    ...
    //二维码
    $url = __HOST__.$file;
    file_put_contents('logoa.png', file_get_contents($url));
    //开始添加水印操作
    // 图片格式png
    ...
    file_put_contents('logoa1.png', file_get_contents($myimg));
    // 图片格式png
    $logo1 = imagecreatefrompng('logoa1.png');
    //拷贝新二维码
    imagecopy($im, $logo1, $width_xx ,$height_yy, 0 ,00, $width_xxx,$width_xxx);
    //保存图片
    imagejpeg($im,BASE_PATH.'/static/1.png');



    //默认背景图片
    $url1 = BASE_PATH.'/static/1.png';
    ...
    file_put_contents('logob.png', file_get_contents($url1a));
    //开始添加水印操作
    // 图片格式jpg
    ...
    $myimg1 = imageUpdatesize(BASE_PATH.$Users['face'],$width_xxx1,$width_xxx1,ss_);
    file_put_contents('logo2.png', file_get_contents($myimg1));
    // 图片格式jpg
    $logo2 = imagecreatefromjpeg('logo2.png');
    //拷贝新二维码
    imagecopy($im1, $logo2, $width_xx1 ,$height_yy1, 0 ,00, $width_xxx1,$width_xxx1);
    //保存图片
    imagejpeg($im1,BASE_PATH.'/static/2.png');
    //文字
    $font = BASE_PATH.'/static/fonts/font20353/2.otf';//字体,字体文件需保存到相应文件夹下
    // print_r($font);die;
    $bigImgPath = BASE_PATH.'/static/2.png';
    ...
    imagefttext($img, $fontsize3, $circleSize, $width_xx2, $height_yy2+$height_xxx3, 0, $font, $username['nickname']);
    //颜色 取消
    //imagefttext($img, $fontsize3, $circleSize, $width_xx2, $height_yy2+$height_xxx3, $black, $font, $username['nickname']);
    imagedestroy($img,BASE_PATH.'/static/3.png');
    //image 图像以 filename 为文件名创建一个 JPEG 图像
    imagejpeg($img,BASE_PATH.'/static/3.png');
    $file = __HOST__.'/static/3.png';
<<<<<<< .mine
||||||| .r2
    ...
    //
//    $size = 30;//字体类型，本例为宋体
//    $font ="c:/windows/fonts/simsun.ttc";//显示的文字
//    $text = $username['nickname'];//创建一个长为500高为80的空白图片
//    $img = imagecreate($width_xxx2, $width_xxx3);//给图片分配颜色
//    imagecolorallocate($img, 0xff, 0xcc, 0xcc);//设置字体颜色
//    $black = imagecolorallocate($img, 0, 0, 0);//将ttf文字写到图片中s
//
//    imagettftext($img, $size, 0, 0, 0, $black, $font, $text);//发送头信息
//
//
//
//
//    imagepng($img,BASE_PATH.'/static/4.png');
