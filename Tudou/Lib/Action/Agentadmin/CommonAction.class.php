...
class CommonAction extends Action
{
    ...
    protected function _initialize(){
        ...
        if(strtolower(MODULE_NAME) != 'login' && strtolower(MODULE_NAME) != 'public') {
            ...
            $this->shop = D('UsersAgentApply')->find(array("where" => array('user_id' => $this->uid,'closed' => 0)));
            if(empty($this->shop)){
                $this->error('该用户没有申请代理', U('login/index'));
            }
            ...
            $this->shop_id = $this->shop['apply_id'];
	   ...
	   //权限获取
        /*--获取当前用户的权限ID-- */
        $name = D('Cityagent')->where(array('agent_id'=>$this->shop[
        'agent_id'],'closed'=>0))->find();
        $Shopgrade = D("ShopauthUser")->where(array("user_id"=>$this->shop['apply_id']))->find();
        $Shopgrade['agent_name'] = $name['agent_name'];
        $Shopgrade['rate'] = $name['rate'];
        // dump($Shopgrade);die;
    ...
    private function parseTemplate($template = ''){
        ...
        define('NOW_PATH', BASE_PATH . '/themes/' . $theme . 'Agentadmin/');
        define('THEME_PATH', BASE_PATH . '/themes/default/Agentadmin/');
        define('APP_TMPL_PATH', __ROOT__ . '/themes/default/Agentadmin/');
        if ('' == $template) {
