...
class  DatasAction extends  CommonAction{
    ...
    public function getCity(){
        $province_id    = $this->_param('province_id');
        $city_data = D('City')->field("city_id, name, lng, lat")->where(array('is_open'=>'1', 'province_id' => $province_id))->select();
    ...
    public function getCityDetail(){
        ...
        $city_data = D('City')->field("lng, lat")->where(array('city_id' => $city_id))->find();
	...
	public function onecity() { //城市
        $name = htmlspecialchars($_GET['name']);
        $data = array();
        $data['city'] = D('City')->where(array('is_open'=>'1'))->order('first_letter asc')->select();
        header("Content-Type:application/javascript");
        echo  'var '.$name.'='.  json_encode($data).';';
        die;
    }
