...
class ArticlereplyAction extends CommonAction
{
    ...
    public function create()
    {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功', U('articlereply/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
        ...
        if ($comment_id = (int) $comment_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('articlereply/index'));
                }
                $this->tuError('操作失败');
            } else {
        ...
        if (is_numeric($comment_id) && ($comment_id = (int) $comment_id)) {
            ...
            foreach ($menu as $val) {
                if ($val['parent_id'] == $comment_id) {
                    $this->tuError('该回复下面还有其他回复');
                }
            }
            $obj->delete($comment_id);
            $this->tuSuccess('删除成功', U('articlereply/index'));
        } else {
    ...
    public function audit($comment_id = 0)
    {
        if (is_numeric($comment_id) && ($comment_id = (int) $comment_id)) {
            ...
            $this->tuSuccess('审核成功', U('articlereply/index'));
        } else {
            $comment_id = $this->_post('comment_id', false);
            if (is_array($comment_id)) {
                $obj = D('Articlecomment');
