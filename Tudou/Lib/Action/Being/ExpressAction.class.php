    ...
    public function create( ){
        if ( $this->isPost( ) ){
            ...
            if ( $express_id = D( "Express" )->add( $data ) ){
                $this->tuSuccess( "发布成功", u( "express/index" ) );
            }
            $this->tuError( "发布失败" );
        }
        else{
    ...
    public function edit($express_id = 0 ){
		 if ($express_id = (int) $express_id) {
            ...
            if (!$detail = $obj->find($express_id)) {
                $this->tuError('请选择要编辑的快递1');
            }
            if ($this->isPost()) {
                $data = $this->editCheck();
                $data['express_id'] = $express_id;
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('express/index'));
                }
                $this->tuError('操作失败');
            } else {
                $this->assign('detail', $detail);
    ...
    public function editCheck( ){
     ...
     public function delete($express_id = 0) {
        if (is_numeric($express_id) && ($express_id = (int) $express_id)) {
            ...
            $this->tuSuccess('删除成功', U('express/index'));
        } else {
