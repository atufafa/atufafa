...
class LinksAction extends CommonAction{
    ...
    public function create(){
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->tuSuccess('添加成功', U('links/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
        ...
        if ($link_id = (int) $link_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $obj->cleanCache();
                    $this->tuSuccess('操作成功', U('links/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    public function delete($link_id = 0){
        if (is_numeric($link_id) && ($link_id = (int) $link_id)) {
            ...
            $this->tuSuccess('删除成功', U('links/index'));
        } else {
	...
	public function audit($link_id = 0){
        if (is_numeric($link_id) && ($link_id = (int) $link_id)) {
            ...
            $this->tuSuccess('审核成功', U('links/index'));
        } else {
            $link_id = $this->_post('link_id', false);
            if (is_array($link_id)) {
                $obj = D('Links');
