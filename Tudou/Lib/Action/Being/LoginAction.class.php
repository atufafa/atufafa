...
class LoginAction extends CommonAction{
    ...
    public function loging(){
        ...
        if($admin['role_id'] != 2){
            session('verify', null);
            $this->tuError('对不起，您不是分站管理员', 2000, true);
        }
        if($admin['is_admin_lock'] == 1){
            ...
            if($present_time_cha < $cha){
                ...
                $this->tuError('您的账户已经被锁定，请' . $echo_time . '秒后登陆', 2000, true);
            }
        }
	   ...
	   //判断IP
        ...
        session('admin', $admin);
        $this->tuSuccess('登录分站成功', U('index/index'));
    }
    public function logout()
    {
        session('admin', null);
        $this->success('退分站出成功', U('login/index'));
    }
