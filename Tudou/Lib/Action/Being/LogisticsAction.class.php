...
class logisticsAction extends CommonAction {
	...
	public function create() {
		if ($this -> isPost()) {
			...
			$obj = D('Logistics');
			if ($obj -> add($data)) {
				$this -> tuSuccess('添加成功', U('logistics/index'));
			}
			$this -> tuError('操作失败');
		}else{
			$this -> display();
		...
		if ($express_id = (int)$express_id) {
			...
			if ($this -> isPost()) {
				...
				if (false !== $obj -> save($data)) {
					$this -> tuSuccess('操作成功', U('logistics/index'));
				}
				$this -> tuError('操作失败');
			} else {
	...
	private function editCheck() {
	 ...
	 public function delete($express_id = 0) {
        if (is_numeric($express_id) && ($express_id = (int) $express_id)) {
            ...
            $this->tuSuccess('删除成功', U('logistics/index'));
        } else {
