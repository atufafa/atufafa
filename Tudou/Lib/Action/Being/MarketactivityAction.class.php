...
class MarketactivityAction extends CommonAction
{
    ...
    public function create()
    {
        if ($this->isPost()) {
            ...
            if ($id = $obj->add($data)) {
                $this->tuSuccess('添加成功', U('marketactivity/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->assign('areas', D('Area')->fetchAll());
    ...
    public function edit($id = 0)
    {
        if ($id = (int) $id) {
            ...
            if (!($detail = $obj->find($id))) {
                $this->tuError('请选择要编辑的商场活动');
            }
            if ($this->isPost()) {
                $data = $this->editCheck($id);
                $data['id'] = $id;
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('marketactivity/index'));
                }
                $this->tuError('操作失败');
            } else {
                $this->assign('areas', D('Area')->fetchAll());
    ...
    public function delete($id = 0)
    {
        if (is_numeric($id) && ($id = (int) $id)) {
            ...
            $this->tuSuccess('删除成功', U('marketactivity/index'));
        } else {
