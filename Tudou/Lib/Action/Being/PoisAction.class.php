...
class PoisAction extends CommonAction {
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            $obj = D('Near');
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功', U('pois/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
        ...
        if ($pois_id = (int) $pois_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false != $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('pois/index'));
                }
                $this->tuError('操作失败');
            } else {
	...
	public function wordcreate() {
        if ($this->isPost()) {
            ...
            if (D('Nearword') ->add($data)) {
                $this->tuSuccess('添加成功', U('pois/word'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    public function wordedit($word_id = 0) {
        if ($word_id = (int) $word_id) {
            ...
            if (!$detail = $obj->find($word_id)) {
                $this->tuError('请选择要编辑的词条');
            }
            if ($this->isPost()) {
				$data = $this->checkFields($this->_post('data', false), $this->word_fields);
				$data['word_id'] = $detail['word_id'];
                if (false != $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('pois/word'));
                }else{
					$this->tuError('操作失败'.$data['price']);
				}
            } else {
                $this->assign('detail', $detail);
                $this->display();
    ...
    public function delete($pois_id = 0) {
        if (is_numeric($pois_id) && ($pois_id = (int) $pois_id)) {
            $obj = D('Near');
            $obj->delete($pois_id);
            $this->tuSuccess('删除成功', U('pois/index'));
        } else {
	...
	public function clean($word_id = 0) {
        if ($word_id = (int) $word_id) {
			...
			if (false != $obj->save($data)) {
				$this->tuSuccess('操作成功', U('pois/stick'));
			}else{
