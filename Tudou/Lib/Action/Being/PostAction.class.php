...
class PostAction extends CommonAction
{
    ...
    public function create(){
        if ($this->isPost()) {
            ...
            $obj = D('Post');
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功', U('post/index'));
            }
            $this->tuError('操作失败');
        } else {
    ...
    public function edit($post_id = 0)
    {
        if ($post_id = (int) $post_id) {
            ...
            if ($detail['city_id'] != $this->city_id) {
                $this->tuError('非法操作', U('post/index'));
            }
            if ($this->isPost()) {
                $data = $this->editCheck();
                $data['post_id'] = $post_id;
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('post/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    public function delete($post_id = 0)
    {
        if (is_numeric($post_id) && ($post_id = (int) $post_id)) {
            ...
            $this->tuSuccess('删除成功', U('post/index'));
        } else {
    ...
    public function audit($post_id = 0)
    {
        if (is_numeric($post_id) && ($post_id = (int) $post_id)) {
            ...
            // print_r($detail);die;
            $this->tuSuccess('审核成功', U('post/index'));
        } else {
            $post_id = $this->_post('post_id', false);
            if (is_array($post_id)) {
                $obj = D('Post');
