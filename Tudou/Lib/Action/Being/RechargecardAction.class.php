...
class RechargecardAction extends CommonAction
{
    ...
    public function create()
    {
        if ($this->isPost()) {
            ...
            if (!empty($card_id)) {
                $this->tuSuccess('添加成功', U('rechargecard/index'));
            } else {
                $this->tuError('操作失败');
            }
        } else {
    ...
    private function createCheck()
    {
        ...
        if (empty($data['value'])) {
            $this->tuError('充值卡面额不能为空');
        }
        $data['name'] = htmlspecialchars($data['name']);
        if (empty($data['name'])) {
            $this->tuError('充值卡名称不能为空');
        }
        $data['num'] = (int) $data['num'];
        if (empty($data['num'])) {
            $this->tuError('添加数量不能为空');
        }
    ...
    public function delete($card_id = 0)
    {
        if (is_numeric($card_id) && ($card_id = (int) $card_id)) {
            $obj = D('Rechargecard');
            $obj->delete($card_id);
            $this->tuSuccess('删除成功', U('rechargecard/index'));
        } else {
