...
class SystemcontentAction extends CommonAction
{
    ...
    public function create()
    {
        if ($this->isPost()) {
            ...
            $obj = D('Systemcontent');
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功', U('systemcontent/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    public function edit($content_id = 0)
    {
        if ($content_id = (int) $content_id) {
            ...
            if (!($detail = $obj->find($content_id))) {
                $this->tuError('请选择要编辑的系统文章');
            }
            if ($this->isPost()) {
                $data = $this->editCheck();
                $data['content_id'] = $content_id;
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('systemcontent/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    public function delete($content_id = 0)
    {
        if (is_numeric($content_id) && ($content_id = (int) $content_id)) {
            ...
            $this->tuSuccess('删除成功', U('systemcontent/index'));
        } else {
