...
class ThreadAction extends CommonAction {
    ...
    public function create() {
        ...
        if ($this->isPost()) {
            ...
            if ($thread_id = $obj->add($data)) {
                $this->tuSuccess('操作成功', U('thread/index'));
            }
            $this->tuError('操作失败');
        } else {
    ...
    private function createCheck() {
        ...
        if (empty($data['thread_name'])) {
            $this->tuError('主题名称不能为空');
        }$data['cate_id'] = (int)$data['cate_id'];
        if (empty($data['cate_id'])) {
            $this->tuError('主题分类不能为空');
        }
        ...
        if (empty($data['banner'])) {
            $this->tuError('请上传banner图');
        }
        if (!isImage($data['photo'])) {
            $this->tuError('banner图格式不正确');
        } 
        $data['intro'] = SecurityEditorHtml($data['intro']);
        if (empty($data['intro'])) {
            $this->tuError('主题简介不能为空');
        }
        if ($words = D('Sensitive')->checkWords($data['intro'])) {
            $this->tuError('主题简介含有敏感词：' . $words);
        } 
    ...
    public function delete($thread_id = 0) {
        ...
        if (is_numeric($thread_id) && ($thread_id = (int) $thread_id)) {
            ...
            $this->tuSuccess('删除成功', U('thread/index'));
        } else {
