<?php
class CommonvehicleAction extends Action{
    protected $uid = 0;
    ...
    protected function _initialize() {
        ...
        if(strtolower(MODULE_NAME) != 'passport' && strtolower(MODULE_NAME) != 'public') {
            if (empty($this->uid)) {
                header("Location: " . U('passport/vehiclelogin'));
                die;
            }
            $Delivery = D('Userspinche')->where(array('user_id' => $this->uid))->find();
			$login_url = U('passport/vehiclelogin');
            if (empty($Delivery)) {
                $this->error('账户不存在', $login_url);
            }
            if ($Delivery['audit'] != 1) {
                $this->error('还没有审核', $login_url);
            }elseif($Delivery['closed'] != 0){
				$this->error('账户已被删除', $login_url);
			}
			$users = D('Users')->where(array('user_id' => $this->uid))->find();
			if ($users['is_lock'] == 1) {
                $this->error('您的账户不安全', $login_url);
            }
            $this->delivery_id = $Delivery['user_id'];//PHP调用
            $this->assign('DELIVERY', $Delivery);//模板调用
        }
