...
class  BookingAction extends  CommonAction{
    ...
    public function order($shop_id){
        ...
        if(!$shop_id = (int)$shop_id){
            ...
            $amount = $total_money + $detail['deposit'];
     ...
     public function orderCreate($shop_id){
        ...
        $is_open = $Booking ->get_times($shop_id,$ding_time);
        if($is_open==1){
            $this->ajaxReturn(array('status'=>'error','msg'=>'商家打烊时间'));
        }
        ...
        $note = htmlspecialchars($this->_param('note'));
         if(!$note){
             $this->ajaxReturn(array('status'=>'error','msg'=>'补充说明不能为空'));
         }
        //订单总额
        $total_money = 0;
        foreach ($dingmenus as $k => $val) {
