...
class FarmAction extends CommonAction {
    ...
    public function order(){
        ...
        $farm_id = I('farm_id',0,'trim,intval');
        if(!$gotime){
            $this->error('没有选择时间',U('farm/detail',array('farm_id'=>$farm_id)));
        }else if(!$name){
            $this->error('没有填写联系人',U('farm/detail',array('farm_id'=>$farm_id)));
        }else if(!$mobile || !isMobile($mobile)){
            $this->error('手机号码不正确',U('farm/detail',array('farm_id'=>$farm_id)));
        }else if(!$pid) {
            $this->error('没有选择套餐',U('farm/detail',array('farm_id'=>$farm_id)));
        }else{
    ...
    public function orderCreate(){
        ...
        if (empty($this->uid)) {
            $this->tuError('您还没有登录',U('passport/login'));
            die;
        }else{
            $data = I('data');
            $gotime = I('gotime',0,'trim');
            ...
            if(!$data['note']){
                $this->tuError('补充说明内容不能为空');
            }
            if(!$data['gotime']){
                $this->tuError('没有选择时间');
            }else if(!$data['name']){
                $this->tuError('没有填写联系人');
            }else if(!$data['mobile'] || !isMobile($data['mobile'])){
                $this->tuError('手机号码不正确！'.$mobile);
            }else if(!$data['pid']){
                $this->tuError('没有选择套餐');
            }else{
                ...
                if($add = D('FarmOrder')->add($data)){
                    $this->tuSuccess('下单成功',U('farm/pay',array('order_id'=>$add)));
                }else{
                    $this->tuError('下单失败!');
                }
            }
