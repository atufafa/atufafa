...
class MallAction extends CommonAction{
    ...
    public function index(){
        ...
        $map = array('closed' => 0, 'audit' => 1,'huodong'=>0, 'city_id' => $this->city_id, 'end_date' => array('EGT', TODAY));
        $linkArr = array();
    ...
    public function shop()
    {
        ...
        $Goods = D('Goods');
        import('ORG.Util.Page');
        $map = array('closed' => 0,'huodong'=>0,'shop_id' => $shop_id, 'audit' => 1);
   ...
   public function get_spec($goods_id){
    ...
    public function order_change() {
 	 ...
 	 foreach($goods_ids as $v){
        ...
        $val = reset($val); 
		//二维数组 取第一个  
		//开发的 其他人可能看不懂 之前是  $num[$val['goods_id']]  这个我前面那个num已经改过了 但是下面的代码不想改了 所以统一赋值一下
		//前面已经通过这个规格的键值来重新传了
	 ...
	 foreach($goods_ids as $v){
		...
		$val = reset($val); //二维数组 取第一个  
         ...
         $is_spec_stock = is_spec_stock($val[goods_id],$val[sky],$num[$val['goods_spec']]);
       	 if(!$is_spec_stock){
       	 	$spec_one_num =  get_one_spec_stock($val[goods_id],$val[sky]);
        	    $this->tuError('亲！规格为<' . $val['key_name']. '>的商品库存不够了,只剩' . $spec_one_num . '件了');
        	}
		
	        if ($val['num'] < $num[$val['goods_spec']]) {
	            $this->tuError('亲！商品<' . $val['title'] . '>库存不够了,只剩' . $val['num'] . '件了');
	        }
	   }
    ...
    public function paycode2(){
        ...
        if ($code == 'wait') {//如果是货到付款
            ...
            $this->tuJump(U('payment/payment', array('log_id' => $detail['log_id'])));
        }
    }
        ...
        foreach ($ordergoods_ids as $k => $v) {
         ...
         $is_spec_stock = is_spec_stock($v[goods_id],$v[key],$v['num']);
            ...
            if ($goods_num['num'] < $v['num']) {
                $this->tuSuccess('商品ID' . $v['goods_id'] . '库存不足无法付款', U('home/mall/index'));
            }
        }
        return false;
    }
