...
class PassportAction extends CommonAction{
    ...
    public function register(){
        if ($this->isPost()) {
            if (isMobile(htmlspecialchars($_POST['data']['account']))) {
                ...
                if (empty($scode2)) {
                    $this->tuError('请获取短信验证码');
                }
                if ($scode != $scode2) {
                    $this->tuError('请输入正确的短信验证码');
                }
            } else {
                $yzm = $this->_post('yzm');
                //是否开启显示验证码1，开启，0关闭
				...
				if($this->_CONFIG['register']['register_mobile']){
					session('verify', null);
                    $this->tuError('当前只能手机号注册，请勿非法操作', 2000, true);
				}
            }
            $data = $this->createCheck();
            $is_agree = $this->_post('is_agree');
            if (!$is_agree) {
                $this->tuError('请同意注册条约!', 2000, true);
            }
            $password2 = $this->_post('password2');
            if ($password2 !== $data['password']) {
                session('verify', null);
                $this->tuError('两次密码不一致', 3000, true);
            }
            //开始其他的判断了
            if (D('Passport')->register($data)) {
                D('Registerip')->delete_register_ip($this->register_ip);
                //删除IP
                $this->tuSuccess('恭喜您，注册成功', U('index/index'));
            }
            $this->tuError(D('Passport')->getError(), 3000, true);
        } else {
            //判断是不是注册会员获取cookie开启
    ...
    public function sendsms(){
        ...
        if ($register['is_lock'] == 1) {
            ...
            //检测IP
            if (empty($var)) {
                $this->tuSuccess('请新休息一下再注册', U('passport/register'));
            } else {
                $sms_yzm = htmlspecialchars($_POST['sms_yzm']);
                if (strtolower($sms_yzm) != strtolower(session('verify'))) {
                    session('verify', null);
                    $this->tuSuccess('请输入正确的验证码后获取手机短信，正在为您跳转', U('passport/register'));
                }
            }
        }
    ...
    public function login(){
        if ($this->isPost()) {
            ...
            if (empty($account)) {
                session('verify', null);
                $this->tuError('请输入用户名!', 2000, true);
            }
            $password = $this->_post('password');
            if (empty($password)) {
                session('verify', null);
                $this->tuError('请输入登录密码!', 2000, true);
            }
            ...
            if (true == D('Passport')->login($account, $password)) {
                $this->tuSuccess('恭喜您登录成功！', $backurl);
            }
            $this->tuError(D('Passport')->getError(), 3000, true);
        } else {
    ...
    private function createCheck(){
        ...
        if (!isMobile($data['account']) && !isEmail($data['account'])) {
            session('verify', null);
            $this->tuError('用户名只允许手机号码或者邮件2!', 2000, true);
        }
        ...
        if (empty($data['password']) || strlen($data['password']) < $register_password) {
            session('verify', null);
            $this->tuError('请输入正确的密码!密码长度必须要在' . $register_password . '个字符以上', 2000, true);
        }
    ...
    public function newpwd()
    {
        ...
        $this->tuSuccess('重置密码成功', U('passport/suc', array('way' => $way)));
    }
    public function suc(){
	...
	public function newpassword() {
		...
		if(!is_valid_email($email,false)) {
         	$this->tuError('邮箱格式错误1');
        }
        if(!$Users = D('Users')->getUserByEmail($email)){
         	$this->tuError('邮箱不正确或者未绑定');
        }
        $yzm = $this->_post('yzm');
        if(strtolower($yzm) != strtolower(session('verify'))){
            session('verify', null);
        ...
        if(D('Passport')->uppwd($Users['account'], '', $password)){
			if(D('Email')->sendMail('email_newpwd', $email, '重置密码', array('newpwd' => $password))){
				$this->tuSuccess('密码重置成功！请登录邮箱查看', U('passport/login'));
			}else{
				$this->tuError('重置密码邮件发送失败，请再试一次');
			}
		}else{
			$this->tuError('重置密码修改数据库失败，请联系管理员解决');
		}
    }
