...
class ThreadAction extends CommonAction {
    ...
    public function collect() {
        ...
        if (D('Threadcollect')->check($thread_id, $this->uid)) {
            if(D('Threadcollect')->where(array('thread_id'=>$thread_id,'user_id'=>$this->uid))->delete()){
                ...
                $this->tuSuccess('取消关注成功！',U('thread/lists',array('cate_id'=>$detail['cate_id'])));
            }
            $this->tuError('取消失败');
        }else{
            $data = array(
            ...
            if (D('Threadcollect')->add($data)) {
                ...
                $this->tuSuccess('恭喜您关注成功！',U('thread/detail',array('thread_id'=>$thread_id)));
            }
            $this->tuError('关注失败');
        }
    }
    ...
    public function fabu(){
        ...
        if (IS_AJAX) {
            ...
            if($users['is_aux'] !=1){
                $this->ajaxReturn(array('status'=>'error','msg'=>'您未实名认证，请前往手机端进行认证'));
            }
    ...
    public function donate(){
        ...
        if (IS_AJAX) {
            ...
            if (D('Users')->addMoney($this->uid,-$money,'打赏帖子'.$detail['title'])) {
                D('Threaddonate')->add($data);
                D('Threadpost')->updateCount($post_id, 'donate_num');
                if($detail['user_id'] >0){
                    D('Users')->addMoney($detail['user_id'],$money,'帖子'.$detail['title'].'被打赏，获得赏金');
                }
