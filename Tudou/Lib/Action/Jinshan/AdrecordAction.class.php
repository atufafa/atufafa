...
class AdrecordAction extends CommonAction {
    ...
    public function audit($id = 0){  
        ...
        if(is_numeric($id) && ($id = (int) $id)){
			...
			$sho=D('Ad')->where(array('site_id'=>$adrecord['site_id'],'city_id'=>$adrecord['city_id'],'user_id'=>$adrecord['user_id']))->find();
//			 if(!empty($sho) && $adrecord['user_id']!=78){
//				$this->tuError('该位置广告已存在');
//			 }
			 
			 
			 if(D('Ad')->where(array('site_id'=>$adrecord['site_id'],'city_id'=>$adrecord['city_id']))->count() >=50){
				$this->tuError('同一位置，轮播图数量超过50个');
			 }
            
     
            $Users = D('Users')->where(array('user_id'=>$adrecord['user_id']))->find();

            if($Users['shopintegral'] < 0 || $Users['shopintegral'] < $adrecord['prestore_integral']){
            ...
            $arr['end_date'] =  $adrecord['end_time'];//购买过后的到期时间 $size
			$arr['goods_id']= $adrecord['goods_id'];
			if($ad_id = D('ad')->add($arr)){
                D('Users')->addShopIntegra($adrecord['user_id'],'-'.$adrecord['prestore_integral'], $intro);//扣除积分
