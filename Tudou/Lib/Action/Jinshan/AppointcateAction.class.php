...
class AppointcateAction extends CommonAction{
    ...
    public function create($parent_id = 0){
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->tuSuccess('添加成功', U('appointcate/index'));
            }
            $this->tuError('操作失败');
        } else {
        ...
        if ($cate_id = (int) $cate_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $obj->cleanCache();
                    $this->tuSuccess('操作成功', U('appointcate/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    public function delete($cate_id = 0){
			...
			if(false == D('Appointcate')->check_cate_id_appoint($cate_id)){
				$this->tuError('当前分类下面还有家政服务删除');
			}
			
            $obj->delete($cate_id);
            $obj->cleanCache();
            $this->tuSuccess('删除成功', U('appointcate/index'));
    }
    public function update(){
        ...
        $obj->cleanCache();
        $this->tuSuccess('更新成功', U('appointcate/index'));
    }
}