...
class AreaAction extends CommonAction{
  ...
  public function business(){
        ...
        $Page = new Page($count, 25);
        $show = $Page->show();
        $list = $Area->where($map)->order(array('business_id' => 'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
		$user_ids = array();
		foreach ($list as $k => $val) {
            $val['business_num'] =D('Area')->get_business_num($val['business_id']);
			$val['shop_num'] = D('Area')->get_shop_num($val['business_id']);
			$user_ids[$val['user_id']] = $val['user_id'];
			$list[$k] = $val;
        }
        $this->assign('list', $list);
