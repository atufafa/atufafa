...
class BillAction extends CommonAction
{
    ...
    public function createtype()
    {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->tuSuccess('添加成功', U('bill/billtype'));
            }
            $this->tuError('操作失败');
        } else {
	 ...
	 private function deletetype($id = 0){
        ...
        if (is_numeric($bill_type_id) && ($bill_type_id = (int) $bill_type_id)) {
            ...
            $obj->cleanCache();
            $this->tuSuccess('删除成功', U('bill/billtype'));
        } else {
    ...
    public function edittype($id = 0)
    {
        if ($id = (int)$id) {
            ...
            if ($this->isPost()) {
                ...
                $this->tuSuccess('操作成功', U('bill/billtype'));
            } else {
