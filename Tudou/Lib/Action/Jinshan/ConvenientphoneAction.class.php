...
class ConvenientphoneAction extends CommonAction{
    ...
    public function create(){
        if ($this->isPost()) {
            ...
            if ($phone_id = $obj->add($data)) {
                $obj->cleanCache();
                $this->tuSuccess('添加成功', U('convenientphone/index'));
            }
            $this->tuError('操作失败');
        } else {
    ...
    private function createCheck() {
        ...
        if (empty($data['phone'])) {
            $this->tuError('便民电话不能为空');
        }
        if (!isMobile($data['phone']) && !isPhone($data['phone'])) {
            $this->tuError('便民电话格式不正确');
        }
        ...
        if ($phone_id = (int) $phone_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $obj->cleanCache();
                    $this->tuSuccess('操作成功', U('convenientphone/index'));
                }
                $this->tuError('操作失败');
            } else {
                $this->assign('areas', D('Area')->fetchAll());
    ...
    public function delete($phone_id = 0){
        if (is_numeric($phone_id) && ($phone_id = (int) $phone_id)) {
            ...
            $obj->cleanCache();
            $this->tuSuccess('删除成功', U('convenientphone/index'));
        } else {
    ...
    public function setaudit($phone_id = 0){
        if (is_numeric($phone_id) && ($phone_id = (int) $phone_id)) {
            ...
            $this->tuSuccess('审核成功', U('convenientphone/index'));
        } else {
            $phone_id = $this->_post('phone_id', false);
            if (is_array($phone_id)) {
                $data['audit'] = '1';
                ...
                $this->tuSuccess('批量审核成功', U('convenientphone/index'));
            }
            $this->tuError('请选择要审核的便民电话管理');
        }
    }
        ...
        if ($phone_id = (int) $phone_id) {
            ...
            if ($this->isPost()) {
                ...
                if (!($res = $obj->where(array('phone_id' => $phone_id, 'community_id' => $community_id))->select())) {
                    $obj->add($data);
                    $this->tuSuccess('操作成功', U('convenientphone/set', array('phone_id' => $phone_id)));
                } else {
                    $this->tuError('该便民电话已存在');
                }
            } else {
        ...
        if ($phone_id = (int) $phone_id) {
            ...
            if ($this->isPost()) {
                ...
                if (!($res = $obj->where(array('phone_id' => $phone_id, 'village_id' => $community_id))->select())) {
                    $obj->add($data);
                    $this->tuSuccess('操作成功', U('convenientphone/setVillage', array('phone_id' => $phone_id)));
                } else {
                    $this->tuError('该便民电话已存在');
                }
            } else {
    ...
    public function cancel($phone_id, $community_id)
    {
        ...
        $obj->cleanCache();
        $this->tuSuccess('取消成功', U('convenientphone/set', array('phone_id' => $phone_id)));
    }
    public function cancelV($phone_id, $village_id)
    {
        ...
        $obj->cleanCache();
        $this->tuSuccess('取消成功', U('convenientphone/setvillage', array('phone_id' => $phone_id)));
    }
}