...
class EduAction extends CommonAction {
   ...
   private $course_create_fields = array('edu_id','title','money', 'photo','type','price','course_price','test_price','cate_id','age_id', 'time_id', 'type_id', 'class_id','class_time', 'course_time', 'apply_time','is_refund', 'views','sale','orderby','audit','details');
   private $course_edit_fields = array('edu_id','title', 'money','photo','type','price','course_price','test_price','cate_id','age_id', 'time_id', 'type_id','class_id', 'class_time', 'course_time', 'apply_time','is_refund', 'views','sale','orderby','audit','details');
        ...
        if ($edu_id = (int) $edu_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('edu/noaudit'));
                }
                $this->tuError('操作失败');
            } else {
 ...
 //教育商家审核
  ...
  //课程列表
    ...
    private function course_createCheck() {
		...
		$data['test_price'] = (float) ($data['test_price']);//试课价格
		if($data['type'] ==1){
