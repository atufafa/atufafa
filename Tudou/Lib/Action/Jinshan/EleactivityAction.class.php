...
public function addtype(){
    if ($this->isPost()) {
        ...
        if ($obj->add($data)) {
            $obj->cleanCache();
            $this->tuSuccess('添加成功', U('eleactivity/typelist'));
        }
        $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    if ($type_id = (int) $type_id) {
        ...
        if ($this->isPost()) {
            ...
            if (false !== $obj->save($data)) {
                $this->tuSuccess('操作成功', U('eleactivity/typelist'));
            }
            $this->tuError('操作失败');
        } else {
...
public function deltype($type_id){
    if (is_numeric($type_id) && ($type_id = (int) $type_id)) {
        $obj = D('ActivityEleStoreMarket');
        $obj->delete($type_id);
        $this->tuSuccess('删除成功', U('eleactivity/typelist'));
    }
...
public function index(){
    ...
    $this->assign('type',D('ActivityEleStoreMarket')->where(['type'=>'ele'])->itemsByIds($type_ids));
    $this->assign('shops', D('Shop')->itemsByIds($shop_ids));
    $this->assign('goods',D('Eleproduct')->itemsByIds($goods_ids));
    $this->assign('start',D('TimeEleStoreMarket')->where(['type'=>1])->itemsByIds($time_ids));
    $this->assign('list', $list);
...
public function edit($id){
    ...
    if ($id = (int) $id) {
        ...
        if ($this->isPost()) {
            ...
            $data['id'] = $id;
            if (false !== $obj->save($data)) {
                $this->tuSuccess('操作成功', U('eleactivity/index'));
            }
            $this->tuError('操作失败');
        } else {
            $detail=$obj->where(['id'=>$id])->find();
            $this->assign('detail',$detail);
            $this->assign('type',D('ActivityEleStoreMarket')->where(['type'=>'ele'])->select());
            $this->assign('start',D('TimeEleStoreMarket')->where(['type'=>1])->select());
            $this->display();
        }
    } else {
        $this->tuError('请选择要编辑的活动类型');
    ...
    private function editsCheck(){
        ...
        if($data['type_id']==3 && empty($data['num'])){
            $this->tuError('请选择售卖份数');
        }
        if($data['type_id']==1 && empty($data['num'])){
            $this->tuError('请选择售卖份数');
        }
    ...
    public function shenhe($id){
        if (is_numeric($id) && ($id = (int) $id)) {
            ...
            $this->tuSuccess('审核成功', U('eleactivity/index'));
        }
    ...
    public function del($id){
        if (is_numeric($id) && ($id = (int) $id)) {
            ...
            $this->tuSuccess('删除成功', U('eleactivity/index'));
        }
    ...
    public function addtime(){
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功', U('eleactivity/timeindex'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    public function edittime($time_id){
        if ($time_id = (int) $time_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('eleactivity/timeindex'));
                }
                $this->tuError('操作失败');
            } else {
                $this->assign('detail', $detail);
    ...
    public function deltime($time_id){
        if (is_numeric($time_id) && ($time_id = (int) $time_id)) {
            $obj = D('TimeEleStoreMarket');
            $obj->delete($time_id);
            $this->tuSuccess('删除成功', U('eleactivity/timeindex'));
        }
