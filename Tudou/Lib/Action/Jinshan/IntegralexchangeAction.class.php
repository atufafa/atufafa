...
class IntegralexchangeAction extends CommonAction
{
    ...
    public function del($exchange_id = 0)
    {
        if (is_numeric($exchange_id) && ($exchange_id = (int) $exchange_id)) {
            $obj = D('Integralexchange');
            $shan=$obj->where(array('exchange_id'=>$exchange_id))->find();
            if($shan['audit']!=4 && $shan['audit']!=0){
                $this->tuError('该商品未完成配送，不能删除');
            }else{
                $obj->save(array('exchange_id' => $exchange_id, 'colse' => 1));
                $this->tuSuccess('删除成功', U('integralexchange/index'));
            }
        } else {
            ...
            if (is_array($exchange_id)) {
                ...
                foreach ($exchange_id as $id) {
                    $obj->save(array('exchange_id' => $id, 'colse' => 1));
                }
                $this->tuSuccess('删除成功', U('integralexchange/index'));
            }
