...
class MarketAction extends CommonAction {
    ...
    public function audit($shop_id = 0){
        $shop_id = (int) $shop_id;
        if($shop_id){
            D('Market')->save(array('shop_id' => $shop_id, 'audit' => 1));
            $this->tuSuccess('审核成功', U('market/index'));
        }else{
            $this->tuError('商家不存在');
        }
    }
    ...
    public function cate_status($cate_id,$status)
    {
        if($cate_id = (int)$cate_id){
            if(false !== (M('Marketcate')->where(['cate_id'=>$cate_id])->save(['status'=>$status]))){
                $this->Success('操作成功');
            }else{
                $this->tuError('操作失败');
            }
    ...
    {
        if($cate_id = (int)$cate_id){
            if(false !== (M('Marketcate')->where(['cate_id'=>$cate_id])->save(['is_look'=>$is_look]))){
                $this->Success('操作成功');
            }else{
                $this->tuError('操作失败');
            }
    ...
    {
        if($cate_id = (int)$cate_id){
            if(false !==($detail = M('Marketcate')->where(['cate_id'=>$cate_id])->find())){
                if($this->isPost()){
                    $data = $this->checkFields($this->_post('data', false), array('cate_name','cate_pic','status','is_look'));
                    // print_r($data);die;
                    if(false !==(M('Marketcate')->where(['cate_id'=>$cate_id])->save($data))){
                        $this->tuSuccess('修改成功',U('market/cate_list'));
                    }else{
                        $this->tuError('修改失败');
                    }
                }else{
    ...
    public function cate_add()
    {
        if($this->isPost()){
            $data = $this->checkFields($this->_post('data', false), array('cate_name','cate_pic','status','is_look'));
            if(false !==(M('Marketcate')->add($data))){
                $this->tuSuccess('新增成功',U('market/cate_list'));
            }else{
                $this->tuError('新增失败');
            }
        }
    ...
    {
        if($cate_id = (int)$cate_id){
            if(false !==(M('Marketcate')->where(['cate_id'=>$cate_id])->delete())){
                $this->Success('删除成功',U('market/cate_list'));
            }else{
                $this->Error('删除失败');
            }
        }else{
    ...
    public function cate_parent($parent_id)
    {
        ...
        if ($this->isPost()) {
            ...
            $obj->cleanCache();
            $this->tuSuccess('更新成功', U('market/cate_list'));
        } else {
            $menu = D('Marketcatelist')->select();
