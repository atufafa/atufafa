...
class MenuAction extends CommonAction{
    ...
    public function index(){
        $menu = D('Menu')->order('orderby','asc')->select();
    ...
    public function create($parent_id = 0){
        if($this->isPost()){
            ...
            if ($obj->add($data)){
                $obj->cleanCache();
                $this->tuSuccess('添加成功', U('menu/index'));
            }
            $this->tuError('操作失败');
        }else{
    ...
    public function action($parent_id = 0){
        ...
        if ($this->isPost()) {
            ...
            $obj->cleanCache();
            $this->tuSuccess('更新成功', U('menu/index'));
        } else {
            $menu = D('Menu')->fetchAll();
            $this->assign('datas', $menu);
    ...
    public function edit($menu_id = 0){
        if ($menu_id = (int) $menu_id) {
            ...
            if ($this->isPost()) {
                ...
                if ($obj->save($data)) {
                    $obj->cleanCache();
                    $this->tuSuccess('操作成功', U('menu/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    public function delete($menu_id = 0) {
        if ($menu_id = (int) $menu_id) {
            ...
            $obj->delete($menu_id);
            $obj->cleanCache();
            $this->tuSuccess('删除成功', U('menu/index'));
        }
        $this->tuError('请选择要删除的菜单');
    }
