...
class PluginAction extends CommonAction
{
    ...
    public function create()
    {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->tuSuccess('添加成功', U('plugin/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    private function createCheck()
    {
        ...
        if (empty($data['plugin_name'])) {
            $this->tuError('插件名称不能为空');
        }
        $data['icon'] = htmlspecialchars($data['icon']);
        if (empty($data['icon'])) {
            $this->tuError('请上传插件图标');
        }
        if (!isImage($data['icon'])) {
            $this->tuError('插件图标格式不正确');
        }
        $data['contents'] = htmlspecialchars($data['contents']);
        if (empty($data['contents'])) {
            $this->tuError('插件说明不能为空');
        }
        ...
        if ($plugin_id = (int) $plugin_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $obj->cleanCache();
                    $this->tuSuccess('操作成功', U('plugin/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    public function delete($plugin_id = 0)
    {
        if (is_numeric($plugin_id) && ($plugin_id = (int) $plugin_id)) {
            ...
            $obj->cleanCache();
            $this->tuSuccess('删除成功', U('plugin/index'));
        } else {
