...
class ShopcateAction extends CommonAction{
    private $create_fields = array('cate_name', 'd1', 'd2', 'd3', 'title', 'orderby','is_weisheng','handle');
    private $edit_fields = array('cate_name', 'd1', 'd2', 'd3', 'title', 'orderby','price','is_weisheng','handle','personal_price','personal_handle');
    ...
    private function createCheck(){
        ...
        $data['price'] = (float)$data['price'];//入驻费用公司
        $data['personal_price'] = (float)$data['personal_price'];//个体入住费
    ...
    private function editCheck(){
        ...
        $data['d3'] = htmlspecialchars($data['d3']);
        $data['title'] = htmlspecialchars($data['title']);
        $data['price'] = (float)$data['price'];//公司入驻费
        $data['personal_price'] = (float)$data['personal_price'];//个体入住费
        $data['orderby'] = (int) $data['orderby'];
        return $data;
    }
        ...
        if ($this->isPost()) {
            ...
            foreach ($old as $key => $val) {
                $obj->save(array(
					...
					'attr_price' => (float) ($val['attr_price']),
