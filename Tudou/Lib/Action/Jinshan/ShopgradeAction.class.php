...
class ShopgradeAction extends CommonAction{
    ...
    public function create(){
        if ($this->isPost()) {
            ...
            $obj = D('Shopgrade');
            if ($obj->add($data)) {
                $list = $obj->where(array("closed"=>0))->order(array("orderby"=>"desc"))->find();
                $data['is_grade_id'] = $list['grade_id'];
                $obj->where(array("grade_id"=>$list['grade_id']))->save($data);
                $obj->cleanCache();
