<?php
class SpecificationsAction extends CommonAction {
    ...
    public function addAttribute(){
        ...
        if(IS_POST){
            ...
            if ($attr_id)  {
                $model->save(); // 写入数据到数据库
                $this->tuSuccess("更新成功!!!",U('attribute',array("type_id"=>$type_id)));
            }
            else{
                $insert_id = $model->add(); // 写入数据到数据库
                $this->tuSuccess("添加成功!!!",U('attribute',array("type_id"=>$type_id)));
            }
    ...
    public function delAttribute($attr_id = 0){
        if (is_numeric($attr_id) && ($attr_id = (int) $attr_id)) {
            ...
            $obj->delete($attr_id);
            $this->tuSuccess('删除成功', U('attribute',array('type_id'=>$_GET['oo'])));
        }
    ...
    public function addSpec(){
        ...
        if(IS_POST){
            ...
            if($type == 1){
                if($TpSpec = D("Integralgoodsspec")->where(array('name'=>$data['name'],'type_id'=>$data['type_id']))->find()){
            ...
            if ($type == 2){
                ...
                $this->action($uid,'');
                $this->tuSuccess('编辑成功', U('specifications/spec'));
            }
            else{
                $insert_id = $model->add($data); // 写入数据到数据库
                //$this->afterSave($insert_id);
                $this->action($uid,$insert_id);
                $this->tuSuccess('添加成功', U('specifications/spec'));
            }
        }
        // 点击过来编辑时
    ...
    public function delGoodsSpecType($id = 0) {
        if ($id = (int) $id) {
            ...
            $obj->delete($id);
            $uid = I('uid');
            $this->tuSuccess('删除成功', U('specifications/addSpec',array('id'=>$uid)));
        }
    ...
    public function delSpec($id=0){
        if (is_numeric($id) && ($id = (int) $id)) {
            ...
            $this->tuSuccess('删除成功',  U('specifications/spec'));
        }
        $this->tuError('请选择要删除的选项');
    ...
    public function delType($id=0){
        if (is_numeric($id) && ($id = (int) $id)) {
            $count = D('Integralgoodsattribute')->where(array('type_id'=>$id))->count();
            if($count > 0){
                $this->tuError('该类型下有商品属性不得删除');
            }
            $obj = D('Integralgoodstype');
            $obj->delete($id);
            $this->tuSuccess('删除成功', U('specifications/type'));
        } else {
