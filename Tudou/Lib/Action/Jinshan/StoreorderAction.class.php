...
class StoreorderAction extends CommonAction{
    public function _initialize(){
        parent::_initialize();
        $this->getCfg = D('Storeorder')->getCfg();
    ...
    public function index(){
        ...
        foreach ($list as $k => $val) {
			...
			$ShopMoney = D('Shopmoney')->where(array('type'=>'store','order_id'=>$val['order_id']))->find();
			if(!empty($ShopMoney)){
        ...
        if (!empty($order_ids)) {
            ...
            $this->assign('eleproducts', D('Storeproduct')->itemsByIds($product_ids));
        }
    ...
    public function detail($order_id = 0){
        ...
        if (!($detail = D('StoreOrder')->find($order_id))) {
            ...
            $detail['user'] = $user;
            $StoreOrderProduct = D('StoreOrderProduct')->where(array('order_id' => $detail['order_id']))->select();
            if ($StoreOrderProduct) {
                $product_ids = array();
                foreach ($StoreOrderProduct as $k => $v) {
                    $product_ids[$v['product_id']] = $v['product_id'];
                }
                $Product = D('StoreProduct')->where(array('product_id' => array('in', $product_ids)))->select();
                $products = array();
                foreach ($Product as $kk => $vv) {
                    $products[$vv['product_id']] = $vv;
                }
                $this->assign('storeorderproduct', $StoreOrderProduct);
                $this->assign('products', $products);
    ...
    public function delete($order_id = 0){
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
            ...
            $this->tuSuccess('取消订单成功', U('storeorder/index'));
        } else {
    ...
    public function tui($order_id = 0){
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
            ...
            if ($detail['status'] == 3) {
                if (D('Storeorder')->save(array('order_id' => $order_id, 'status' => 4))) {
                    $obj = D('Users');
                    if ($detail['need_pay'] > 0) {
                        ...
                        $obj->addMoney($detail['user_id'], $detail['need_pay'], '便利店退款');
                    }
                }
            }
        } else {
        ...
        $this->tuSuccess('退款成功', U('storeorder/index'));
    }
