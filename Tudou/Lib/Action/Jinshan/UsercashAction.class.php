...
class UsercashAction extends CommonAction{
   ...
   //   print_r($list);
    ...
    public function gold(){
		...
		$this->assign('shop_cash', round($shop_cash = $Userscash->where(array('type' => shop,'status' =>1))->sum('gold'),2));
		$this->assign('shop_cash_commission', round($shop_cash_commission = $Userscash->where(array('type' => shop,'status' =>1))->sum('commission'),2));
        $this->assign('list', $list);
        $this->assign('page', $show);
        $this->display();
    }
