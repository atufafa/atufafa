...
class UsersauxAction extends CommonAction{
    ...
    private $pinche_fields = array('user_id','city_id', 'area_id', 'business_id','team_id', 'jury_id', 'group_id', 'card_photo', 'name', 'mobile','card_id','addr_str', 'addr_info', 'guarantor_name', 'guarantor_mobile','jia_photo','bao_photo','car_photo','xing_photo');
  ...
  //拼车司机认证审核     ---- 新增
    public function pincheCard()
    {
        $Usersaux = M('UsersPinche');
        import('ORG.Util.Page');
        $map = array('closed' => 0);
        if ($keyword = $this->_param('keyword', 'htmlspecialchars')) {
            $map['name|mobile|card_id|guarantor_name|guarantor_mobile'] = array('LIKE', '%' . $keyword . '%');
        ...
        if (isset($_GET['audit']) || isset($_POST['audit'])) {
            ...
            $map['audit'] = array('IN',array(0,1,-1));
            $this->assign('audit', 999);
        }
        if ($user_id = (int) $this->_param('user_id')) {
            $map['user_id'] = $user_id;
    ...
    //拼车审核
    public function pinche_audit($user_id = 0){
        $user_id = (int) $user_id;
        $obj = M('UsersPinche');
        if($user_id){
            if($detail = M('UsersPinche')->find($user_id)){
                $data['is_pinche'] = 1;
                if(D('Users')->where(array('user_id'=>$user_id))->save($data)){
                    if($obj->where(array('user_id' => $user_id))->save(array('user_id' => $user_id, 'audit' => 1))){
                        $this->tuSuccess('审核成功', U('usersaux/pincheCard'));
                    }else{
                        $this->tuError('更新审核信息失败');
                    }
                }else{
    ...
    //拒绝拼车审核
     public function pinche_refuse($user_id = 0){
        $obj = M('UsersPinche');
         if (is_numeric($user_id) && ($user_id = (int) $user_id)) {
            if ($this->isPost()) {
                $reason = htmlspecialchars($this->_param('reason'));
                ...
                $obj->where(array('user_id' => $user_id))->save(array('user_id' => $user_id, 'audit' => -1,'reason'=>$reason));
                $this->tuSuccess('操作成功', U('usersaux/pincheCard'));
            }else{
                $this->assign('user_id',$user_id);
                $this->display();
            }
    ...
    //拼车审核
    public function shenhe($user_id = 0){
        $user_id = (int) $user_id;
        $obj = M('UsersPinche');
        if($user_id){
            if($detail = M('UsersPinche')->find($user_id)){
                    if($obj->where(array('user_id' => $user_id))->save(['shenghe' => 1])){
                        $this->tuSuccess('审核成功', U('usersaux/pincheCard'));
                    }else{
                        $this->tuError('更新审核信息失败');
                    }
    ...
    //拒绝拼车审核
    public function jujue($user_id = 0){
        $obj = M('UsersPinche');
        if (is_numeric($user_id) && ($user_id = (int) $user_id)) {
            if ($this->isPost()) {
                $reason = htmlspecialchars($this->_param('reason'));
                ...
                $obj->where(array('user_id' => $user_id))->save(array('shenghe' => 2,'liyou'=>$reason));
                $this->tuSuccess('操作成功', U('usersaux/pincheCard'));
            }else{
                $this->assign('user_id',$user_id);
                $this->display();
    ...
    //拼车认证信息删除      ----新增
    public function pinche_delete($user_id = 0){
        if (is_numeric($user_id) && ($user_id = (int) $user_id)) {
            $obj = M('UsersPinche');
            $obj->save(array('user_id' => $user_id, 'closed' => 1));
            D('Users')->save(array('user_id' => $user_id, 'is_pinche' => 0));
            $this->tuSuccess('删除成功', U('usersaux/pincheCard'));
        } else {
            $user_id = $this->_post('user_id', false);
            if (is_array($user_id)) {
                $obj = M('UsersPinche');
                foreach ($user_id as $id) {
                    $obj->save(array('user_id' => $id, 'closed' => 1));
                    D('Users')->save(array('user_id' => $id, 'is_pinche' => 0));
                }
                $this->tuSuccess('批量删除成功', U('usersaux/pincheCard'));
            }
            $this->tuError('请选择要删除的会员认证');
        }
    }
    //拼车认证信息编辑    ----新增
    public function pinche_edit($user_id = 0){
        if ($user_id = (int) $user_id) {
            $obj = M('UsersPinche');
            if (!($detail = $obj->find($user_id))) {
                $this->tuError('请选择要编辑的认证信息');
            }
            if ($this->isPost()) {
                $data = $this->pinche_editCheck();
                $data['user_id'] = $user_id;
    ...
    //拼车认证信息过滤
    private function pinche_editCheck(){
