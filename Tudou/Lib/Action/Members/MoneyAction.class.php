...
class MoneyAction extends CommonAction{
      ...
      public function exchange(){
        if($this->isPost()){
			...
			if (false == D('Users')->check_integral_buy($integral_buy)) {
				$this->tuError('网站后台积分设置不合法，请联系管理员');
			}
            ...
            $this->tuSuccess('您成功兑换余额'.round($money,2).'元',U('logs/moneylogs'));
        }else{
             $this->display();
        }
    }
    ...
    public function jingjiapay(){
        ...
        $status = M('Jingjia')->where(['id'=>1])->find();
         if($status['status'] ==1){
        	$this->error('未开启竞价，请稍后重试');
        }
        if($status['min_money'] >$money){
        	$this->error('充值最低金额￥'.$status['min_money']);
        }
    ...
    public function recharge(){
        ...
        if ($this->isPost()) {
            ...
            if (empty($card_key)) {
                $this->tuError('充值卡号不能为空');
            }
            if (!($detail = D('Rechargecard')->where(array('card_key' => $card_key))->find())) {
                $this->tuError('该充值卡不存在');
            }
            if ($detail['is_used'] == 1) {
                $this->tuError('该充值卡已经使用过了');
            }
            ...
            if (D('Rechargecard')->save(array('card_id' => $detail['card_id'], 'is_used' => 1))) {
                ...
                //微信通知
                $this->tuSuccess('充值成功', U('money/recharge'));
            }
        } else {
            $this->display();
        }
				...
				if(D('Users')->addMoney($users['user_id'],$money,$intro)){
					...
					if($log_id){
						...
						if(D('Users')->addMoney($this->uid,-$receive_money,$intro2)){
							$this->tuSuccess('恭喜您转账成功',U('money/transfer')); 
						}else{
							$this->tuError('操作失败');
						}
					}else{
      ...
      public function integral(){
        if($this->isPost()){
			...
			if($users){
				...
				$this->tuSuccess('恭喜您转账积分成功',U('logs/integral')); 
			}else{
				$this->tuError('没有找到会员');
			}
        }else{
   ...
   //检测扫码支付支付状态
	 ...
	 public function capitalpay(){
        ...
        $moneys=$this->_CONFIG['cash']['user_capitalpay'];

        if($money < $moneys){
        	$this->error('充值最低金额￥'.$moneys);
        }
     ...
     public function chongzhi(){
         ...
         $moneys=$this->_CONFIG['cash']['return_recharge_money'];
