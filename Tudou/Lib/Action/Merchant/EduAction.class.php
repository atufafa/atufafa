...
class EduAction extends CommonAction {  
   ...
   private $course_create_fields = array('edu_id','money','title', 'photo','type','price','course_price','cate_id','age_id', 'time_id', 'type_id', 'class_id','class_time', 'course_time', 'apply_time','is_refund','details');
   private $course_edit_fields = array('edu_id','money','title', 'photo','type','price','course_price','cate_id','age_id', 'time_id', 'type_id','class_id', 'class_time', 'course_time', 'apply_time','is_refund','details');
  ...
  //       }
   ...
   //课程列表
    ...
    private function course_createCheck() {
		...
		$data['course_price'] = (float) ($data['course_price']);//完整课程价格
		if($data['type'] ==0){
   ...
   //课程取消订单
    ...
    //确认课程订单
    public function edit_status($order_id,$order_status)
    {
        $this->check_edu();
        $map2 = array('shop_id'=>$this->shop_id,'order_id'=>$order_id,'start_time'=>array('gt',0),'end_time'=>array('gt',0));
        $count = M('edu_class_record')->where($map2)->count();
        ...
        if($order_id = (int) $order_id){
            if(!$order = D('EduOrder')->find($order_id)){
                ...
                if($count<$order['class_time']) {
                    $this->tuError('订单确认失败,授课课时未完成');
                }
                $res = M('edu_order')->where(array("order_id" => $order_id))->save(array("order_status" => $order_status, "is_used_code" => 1, 'is_used_time' => time()));
                if (!empty($res)) {
                    $this->tuSuccess('订单确认成功', U('edu/index'));
                } else {
                    $this->tuError('订单确认失败,授课课时未完成');
                }
            }
    ...
    public function edureply($comment_id){
        ...
        $detail = D('EduComment')->where(array('comment_id'=>$comment_id))->find();
        ...
        if ($this->isPost()) {
            if ($reply = $this->_param('reply', 'htmlspecialchars')) {
                ...
                if (D('EduComment')->where(array('comment_id'=>$comment_id))->save($data)) {
                    $this->tuSuccess('回复成功', U('edu/edu'));
                }
            }
            $this->tuError('请填写回复');
        } else {
    ...
    public function comment_delete($comment_id =0)
    {
        ...
        if ($comment_id = (int) $comment_id){
            $obj = D('EduComment');
            $detail = D('EduComment')->where(array('comment_id' => $comment_id, 'shop_id' => $this->shop_id))->find();
            if (!$detail){
                $this->tuError('点评记录不存在');
            }
            if($obj->delete($comment_id)){
                $this->tuSuccess('删除成功', U('edu/edu'));
            }
            $this->tuError('操作失败');
        } else {
    ...
    //授课详情
    public function class_logs($order_id){
        $EduOrder = M('edu_class_record');
        import('ORG.Util.Page');
        $map = array('shop_id'=>$this->shop_id,'order_id'=>$order_id);
        $count = $EduOrder->where($map)->count();
        $map2 = array('shop_id'=>$this->shop_id,'order_id'=>$order_id,'start_time'=>array('gt',0),'end_time'=>array('gt',0));
