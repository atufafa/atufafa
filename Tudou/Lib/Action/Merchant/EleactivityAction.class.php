...
class EleactivityAction extends CommonAction{
    ...
    public function _initialize()
    {
        ...
        $is_ipen=$config['ele']['is_open'];
        if($is_ipen==0){
            $this->tuError('此活动暂未开放');
        }

        $Shopping=D('GoodsEleStoreMarket')->where(['shop_id'=>$this->shop_id,'type'=>1,'type_id'=>1,'audit'=>1,'closed'=>0])->select();
    ...
    private $create_fields = array('type_id','time_number','is_shop','is_receive','time_id','spike_time','end_times', 'shop_id','product_id','type',  'price',  'end_date', 'num','link_url','type2_id');
    //列表
    public function index(){
        $Activity = D('GoodsEleStoreMarket');
        import('ORG.Util.Page');
        $map = array('closed' => 0,'shop_id'=>$this->shop_id,'type'=>1);
        ...
        $this->assign('type', D('ActivityEleStoreMarket')->where(['type'=>'ele'])->itemsByIds($type_id));
        $this->assign('time',D('TimeEleStoreMarket')->where(['type'=>1])->itemsByIds($time_ids));
        $tuanlist = D('Eleproduct')->itemsByIds($product_id);
    ...
    public function create(){
        if ($this->isPost()) {
            ...
            $cunz=$obj->where(['product_id'=>$data['product_id'],'type_id'=>$data['type_id'],'closed'=>0,'audit'=>1,'type'=>1])->find();
            if(!empty($cunz)){
                $this->tuError('该商品已存在该活动');
            }
            if ($obj->add($data)) {
                D('Eleproduct')->where(['shop_id'=>$this->shop_id,'product_id'=>$data['product_id']])->save(['price'=>$data['price'],'type_id'=>$data['type_id'],'num'=>$data['num'],'num1'=>$data['num'],'is_huodong'=>1]);
                $this->tuSuccess('添加成功', U('eleactivity/index'));
            }
            $this->tuError('操作失败');
        } else {
            ...
            $tuanlist = D('Eleproduct')->where(array('shop_id' => $shop_id))->select();
            $this->assign('goods',$tuanlist);
            $this->assign('time',D('TimeEleStoreMarket')->where(['type'=>1])->select());
    ...
    private function createCheck()
    {
        ...
        $timenum=D('TimeEleStoreMarket')->where(['type'=>1,'time_id'=>$data['time_id']])->find();
        $data['time_number']=$timenum['time_name'];
        //限时秒杀
        $data['end_times'] = (int) $data['end_times'];
        if($data['type_id']==2 && empty($data['end_times'])){
            $this->tuError('请输入小时数');
        }
        if($data['type_id']==2){
    ...
    public function delete($id = 0){
        ...
        if(!empty($id)){
            ...
            D('Eleproduct')->where(['product_id'=>$detail['product_id'],'shop_id'=>$this->shop_id])->save(['closed'=>1,'is_huodong'=>0,'type_id'=>0,'num'=>0,'num1'=>0]);
            $this->tuSuccess('删除成功,该产品以下架', U('eleactivity/index'));
        }else{
    ...
    public function shopcreate(){
        ...
        if($this->ispost()){
            $obj=D('ShoppingEleStoreMarket');
            $datail=$obj->where(['shop_id'=>$this->shop_id,'audit'=>1,'close'=>0,'type'=>1])->select();
            if(!empty($datail)){
                 $this->tuError('您发布的抢购卷未过期，不能重复发布');
            }
            $num=I('post.num');
            $end=$config['ele']['shopping_time'];
            ...
            $data['type']=1;
            if($obj->add($data)){
                $this->tuSuccess('发布成功', U('eleactivity/shopping'));
            }
        $this->tuError('发布失败');
        }else{
    ...
    public function del($id){
        ...
        if(!empty($id)){
            ...
            $obj->where(['id'=>$id])->save(['closed'=>1]);
            $this->tuSuccess('删除成功', U('eleactivity/shoppinglist'));
        }else{
            $this->tuError('请选择要删除的活动');
        }
    }
