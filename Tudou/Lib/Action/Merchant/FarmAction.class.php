...
class FarmAction extends CommonAction {
    ...
    public function index(){
        ...
        $show       = $Page->show();
        $list = $fo->where($map)->order('order_id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function fram_dianp()
    {
        ...
        foreach($list as $k => $v){
            $p = D('FarmOrder') -> where(array('order_id'=>$v['order_id'])) -> find();
            $user = D('Users')->where(['user_id'=>$v['user_id']])->find();
            $package = D('FarmPackage')->where(['pid'=>$p['pid']])->find();
            // echo D('FarmPackage')->getLastSql();die;
            $list[$k]['amount'] = $p['amount']; 
            $list[$k]['title'] = $package['title']; 
            $list[$k]['jiesuan_price'] = $p['jiesuan_amount']; 
            $list[$k]['nickname'] = $user['nickname'];
            $list[$k]['phone'] = $user['mobile'];
        }
        // dump($list);die;
        $this->assign('list',$list);
        $this->assign('page',$show);
    ...
    public function reply_comment($comment_id)
    {
        if($this->isPost()){
            ...
            if($comment_id = (int) $comment_id){
                if(!$order = D('FarmComment')->find($comment_id)){
                    $this->tuError('该评论不存在或已删除');
                }else{
                    if($order['shop_id']!=$shop_id){
                        $this->tuError('您不能操作其他商家的评论');
                    }
                    // print_r($_POST);die;
                    $data['reply'] = $_POST['reply'];
                    $data['reply_time'] = NOW_TIME;
                    if(false !== D('FarmComment')->where(['comment_id'=>$comment_id])->save($data)){
                        $this->tuSuccess('回复评论成功',U('farm/fram_dianp'));
                    }else{
                        $this->tuError('回复评论失败');
                    }
                }
            }else{
                $this->tuError('请选择要回复的评论信息');
            }
        }else{
            $this->assign('detail',D('FarmComment')->find($comment_id));
            $this->display();
        }
    }
    //农家乐商家评论删除
    public function delete_comment($comment_id)
    {
        ...
        if($comment_id = (int) $comment_id){
            if(!$order = D('FarmComment')->find($comment_id)){
                ...
                if(false !== D('FarmComment')->where(['comment_id'=>$comment_id])->delete()){
                    $this->tuSuccess('删除评论成功',U('farm/fram_dianp'));
                }else{
                    $this->tuError('删除评论失败');
                }
            }
            ...
            if (empty($Farm)) {
                ...
                if(false !== $obj->save($data)){
                   D('FarmPics')->where(array('farm_id'=>$farm_id))->delete();
                    ...
                    foreach($group_id as $key=>$val){
                        D('FarmPlayAttr')->add(array('shop_id'=>$data['shop_id'],'attr_id'=>$val));
                    }
                    D('FarmGroupAttr')->where(array('shop_id'=>$data['shop_id']))->delete();
                    foreach($cate_id as $k=>$v){
                        D('FarmGroupAttr')->add(array('shop_id'=>$data['shop_id'],'attr_id'=>$v));
                    }
...
//添加套餐
    ...
    public function reply($order_id)
    {
        // dump($order_id);die;
        $farm_id = $this->check_farm();
        if($this->isPost()){
            if($order_id = (int) $order_id){
                if(!$order = D('FarmOrder')->find($order_id)){
                   ...
                   // dump($_POST);die;
                    if($_POST['farm_juan']!=$order['farm_juan']){
                        $this->tuError('错误的电子券，请重新输入');
                    }
                    if(false !== D('FarmOrder')->complete($order_id)){
                        $this->tuSuccess('订单操作成功',U('farm/index'));
                    }else{
            ...
            $this->assign('detail',D('FarmOrder')->find($order_id));
            $this->display();
        }
    }
