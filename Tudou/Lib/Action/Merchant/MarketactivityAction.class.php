...
class MarketactivityAction extends CommonAction{
    ...
    public function _initialize()
    {
        ...
        $is_ipen=$config['market']['is_open'];
        if($is_ipen==0){
            $this->tuError('此活动暂未开放');
        }

        $Shopping=D('GoodsEleStoreMarket')->where(['shop_id'=>$this->shop_id,'type'=>3,'type_id'=>7,'audit'=>1,'closed'=>0])->select();
    ...
    public function index(){
        ...
        $this->assign('type', D('ActivityEleStoreMarket')->where(['type'=>'market'])->itemsByIds($type_id));
        $this->assign('time',D('TimeEleStoreMarket')->where(['type'=>3])->itemsByIds($time_ids));
        $tuanlist = D('Marketproduct')->itemsByIds($product_id);
    ...
    public function create(){
        if ($this->isPost()) {
            ...
            $cunz=$obj->where(['product_id'=>$data['product_id'],'type_id'=>$data['type_id'],'closed'=>0,'audit'=>1,'type'=>3])->find();
            if(!empty($cunz)){
                $this->tuError('该商品已存在该活动');
            }
            if ($obj->add($data)) {
                D('Marketproduct')->where(['shop_id'=>$this->shop_id,'product_id'=>$data['product_id']])
            ...
            $tuanlist =D('Marketproduct')->where(array('shop_id' => $shop_id))->select();
            $this->assign('goods',$tuanlist);
            $this->assign('time',D('TimeEleStoreMarket')->where(['type'=>3])->select());
    ...
    private function createCheck()
    {
        ...
        $timenum=D('TimeEleStoreMarket')->where(['type'=>3,'time_id'=>$data['time_id']])->find();
        $data['time_number']=$timenum['time_name'];

        //限时秒杀
        $data['end_times'] = (int) $data['end_times'];
        if($data['type_id']==8 && empty($data['end_times'])){
            $this->tuError('请输入小时数');
        }
        if($data['type_id']==8){
    ...
    public function delete($id = 0){
        ...
        if(!empty($id)){
            ...
            D('Marketproduct')->where(['product_id'=>$detail['product_id'],'shop_id'=>$this->shop_id])
                ->save(['closed'=>1,'type_id'=>0,'num'=>0,'num1'=>0,'is_huodong'=>0]);
            $this->tuSuccess('删除成功,该产品以下架', U('marketactivity/index'));
        }else{
    ...
    public function shopcreate(){
        ...
        if($this->ispost()){
            $obj=D('ShoppingEleStoreMarket');
            $datail=$obj->where(['shop_id'=>$this->shop_id,'audit'=>1,'close'=>0,'type'=>3])->select();
            if(!empty($datail)){
                $this->tuError('您发布的抢购卷未过期，不能重复发布');
            }
            $num=I('post.num');
            $end=$config['market']['shopping_time'];
            ...
            $data['type']=3;
            if($obj->add($data)){
                $this->tuSuccess('发布成功', U('marketactivity/shopping'));
            }
            $this->tuError('发布失败');
        }else{
    ...
    public function del($id){
        ...
        if(!empty($id)){
            ...
            $obj->where(['id'=>$id])->save(['closed'=>1]);
            $this->tuSuccess('删除成功', U('marketactivity/shoppinglist'));
        }else{
            $this->tuError('请选择要删除的活动');
        }
    }
