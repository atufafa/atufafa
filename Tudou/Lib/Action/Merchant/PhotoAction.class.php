...
class PhotoAction extends CommonAction{
    ...
    public function update(){
        ...
        $this->tuSuccess('更新成功', U('photo/index'));
    }
    ...
    public function photo_create() {
        if ($this->isPost()) {
            ...
            $obj = D('Shoppic');
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功，请等待网站管理员审核', U('photo/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
   ...
   private function photo_createCheck(){
        ...
        if (empty($data['photo'])) {
            $this->tuError('请上传环境图图片');
        }
        if (!isImage($data['photo'])) {
            $this->tuError('环境图图片格式不正确');
        }
    ...
    public function delete(){
        ...
        if (!empty($detail) && $detail['shop_id'] == $this->shop_id) {
            $obj->delete($pic_id);
            $this->tuSuccess('删除成功', U('photo/index'));
        }
        $this->tuError('你懂的');
    }
    ...
    public function updatebanner(){
        ...
        $this->tuSuccess('更新成功', U('photo/banner'));
    }
    public function deletebanner(){
        ...
        if (!empty($detail) && $detail['shop_id'] == $this->shop_id) {
            ...
            $this->tuSuccess('删除成功', U('photo/banner'));
        }
        $this->tuError('你懂的');
    }
    public function updatebanner1(){
        ...
        $this->tuSuccess('更新成功', U('photo/banner1'));
    }
    public function deletebanner1(){
        ...
        if (!empty($detail) && $detail['shop_id'] == $this->shop_id) {
            $obj->delete($banner_id);
            $this->tuSuccess('删除成功', U('photo/banner1'));
        }
        $this->tuError('你懂的');
    }
}