...
class  PoisAction extends CommonAction{
		...
		if($money < $word[price]){
			...
			if(empty($pois)){
				...
				if(D('Users')->addMoney($this->uid, -$eprice ,'购买黄页关键词包年服务，关键词：'.$word['text'])){
					...
					$this->tuSuccess('您为【'.$pois['name'].'】购包年关键词【'.$word['text'].'】成功！',U('pois/stick'));
				}else{
					$this->tuError('购买错误，没有付款成功');
				}
			}
		}
	}
    ...
    public function create() {
        if ($this->isPost()) {
			...
			if (empty($data['photo'])) {
				$this->tuError('请上传黄页图片');
			}
			if (!isImage($data['photo'])) {
				$this->tuError('黄页图片格式不正确');
			} 
			$data['status'] = -1;	
			...
			$obj = D('Near');
		if ($obj->add($data)) {
			$this->tuSuccess('添加成功等待系统审核', U('pois/me'));
		}
		$this->tuError('操作失败');
			...
			if($pois['orderby'] >= 2){
				if(D('Users')->addMoney($this->uid, -100,'提升黄页权重1点')){
				   D('Near')->save(array('orderby'=>$pois[orderby] - 1,'pois_id'=>$pois_id));
					$this->tuSuccess('您为'.$pois['name'].'提升了1点权重！',U('pois/me'));
				}
			}else{
				 $this->tuError('您排名很高了，无需购买');
			}
		}
	}
    ...
    public function lead($pois_id = 0) {
        ...
        if (empty($detail)) {
            $this->tuError('请选择需要编辑的内容操作');
        }
        if (!empty($detail['shop_id'])) {
            $this->tuError('该黄页已经名花有主');
        }
		...
		if($count >0){
			$this->tuError('您还有未审核的黄页，请等待审核完毕后再认领');
		}else{
			$obj->save(array('shop_id'=>$this->shop_id,'status'=> $map['status'],'pois_id'=>$pois_id));
			$this->tuSuccess('恭喜您认领成功，审核完毕后该黄页就属于您了！',U('pois/me'));
		}
	}
