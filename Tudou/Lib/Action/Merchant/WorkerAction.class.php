...
class WorkerAction extends CommonAction {
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if (!empty($worker_id)) {
				...
				if($msg_id){
					$this->tuSuccess('添加成功，已经为该用户发送系统信息，等待他确认', U('worker/index'));
				}else{
					$this->tuError('操作失败');
				}
    ...
    public function edit($worker_id = 0) {
        ...
        if ($this->isPost()) {
            ...
            if (false !== $obj->save($data)) {
                $this->tuSuccess('操作成功', U('worker/index', array('worker_id' => $worker_id)));
            }
            $this->tuError('操作失败');
        } else {
            $this->assign('detail', $detail);
    ...
    private function editCheck() {
        ...
        if (empty($data['work'])) {
            $this->tuError('员工职务不能为空');
        }
		$data['addr'] = htmlspecialchars($data['addr']);
        if (empty($data['addr'])) {
            $this->tuError('联系地址不能为空');
        }
