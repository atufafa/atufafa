<?php
class ZeroelementAction extends CommonAction{
    ...
    public function spec(){
        ...
        $ts = D('Zeroelementspec')->getTableName();
        $tgt = D('Zeroelementtype')->getTableName();
        $tsi = D('Zeroelementspecitem')->getTableName();
        ...
        $show = $Page->show();
        $specList = D('Zeroelementspec')->where($where)
    ...
    public function addspec(){
        ...
        if(IS_POST){
            ...
            if($this->shop_id){
                ...
                if ($type == 2){
                    ...
                    $this->tuSuccess('编辑成功', U('zeroelement/spec'));
                }
                else{
                    $insert_id = $model->add($data); // 写入数据到数据库
                    //$this->afterSave($insert_id);
                    $this->action($_POST['id'],$insert_id);
                    $this->tuSuccess('添加成功', U('zeroelement/spec'));
                }
            }
            else
            {
                $this->tuSuccess('操作失败', U('zeroelement/spec'));
            }
        }
        ...
        $this->assign('uid',$id);
        $goodsTypeList = D('Zeroelementtype')->where(array('shop_id'=>$this->shop_id,'recycle'=>array('EXP','IS NULL')))->select();
...
//        $arr = get_id_val($arr, 'id','item');
    ...
    public function del($id = 0) {
        if ($id = (int) $id) {
            ...
            $uid = I('uid');
            $this->tuSuccess('删除成功', U('zeroelement/addspec',array('id'=>$uid)));
        }
    ...
    public function delete($id=0){
        if($id = (int) $id){
            ...
            //$obj->delete($id);
            $this->tuSuccess('删除成功', U('zeroelement/spec'));
        }
    }
    ...
    public function reductionspecifications($id=0){
        if($id = (int) $id){
            ...
            $data['recycle']=null;
            $obj->where(array('id'=>$id))->save($data);
            $this->tuSuccess('还原成功', U('zeroelement/spec'));
        }
    }
    ...
    public function addbranch(){
        if ($this->isPost()) {
            ...
            $obj = D('Zeroelementcate');
            $data['shop_id'] = $this->shop_id;
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功', U('zeroelement/branch'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    private function createCheck()
    {
        ...
        $detail = D('Zeroelementcate')->where(array('shop_id' => $this->shop_id, 'cate_name' => $data['cate_name']))->select();
        if (!empty($detail)) {
        ...
        if ($cate_id = (int) $cate_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('zeroelement/branch'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    public function  delclass($cate_id=0){
        if($cate_id = (int) $cate_id){
            ...
            $this->tuSuccess('删除成功', U('zeroelement/branch'));
        }
    }
    ...
    public function reductionclass($cate_id=0){
        if($cate_id = (int) $cate_id){
            ...
            $data['recycle']=null;
            $obj->where(array('cate_id'=>$cate_id))->save($data);
            $this->tuSuccess('还原成功', U('zeroelement/branch'));
        }
    }
        ...
        if(IS_POST){
            ...
            if($this->shop_id){
                ...
                if ($type == 2){
                    ...
                    $this->tuSuccess('编辑成功', U('zeroelement/attr'));
                }
                else{
                    $data['attr_values']=trim($ii);
                    $insert_id = $model->add($data); // 写入数据到数据库
                    $this->tuSuccess('添加成功', U('zeroelement/attr'));
                }
            }
            else
            {
                $this->tuSuccess('操作失败', U('zeroelement/attr'));
            }
        }
    ...
    public function delattrs($attr_id=0){
        if($attr_id = (int) $attr_id){
            ...
            //$obj->delete($attr_id);
            $this->tuSuccess('删除成功', U('zeroelement/attr'));
    ...
    public function reductionattr($attr_id=0){
        if($attr_id = (int) $attr_id){
            ...
            $data['recycle']=null;
            $obj->where(array('attr_id'=>$attr_id))->save($data);
            $this->tuSuccess('还原成功', U('zeroelement/attr'));
        }
    }
    ...
    public function addtype(){
        ...
        if(IS_POST){
            ...
            if($id){
                ...
                $data['id']=$id;
                $model->save($data);
                $this->tuSuccess("编辑成功!",U('zeroelement/type'));
            }else{
                $data['shop_id']=$this->shop_id;
                $data['name']=I('name');
                $model->add($data);
                $this->tuSuccess("添加成功!",U('zeroelement/type'));
            }
        }
    ...
    public function deltypes($id = 0){
        if($id = (int) $id){
            ...
            $data['recycle']=1;
            $obj->where(array('id'=>$id))->save($data);
            $this->tuSuccess('删除成功', U('zeroelement/type'));
        }
    }
    ...
    public function reduction($id = 0){
        if($id = (int) $id){
            ...
            $data['recycle']=null;
            $obj->where(array('id'=>$id))->save($data);
            $this->tuSuccess('还原成功', U('zeroelement/type'));
        }
    }
    ...
    public function goodsjingjia()
    {
        ...
        foreach($list as $k => $val){
            $goods = D('Pindan')->where(['tuan_id'=>$val['goods_id']])->find();
    ...
    public function jingjiadel($bid_id)
    {
        ...
        $this->tuSuccess('删除成功', U('zeroelement/goodsjingjia'));
    }
