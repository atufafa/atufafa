...
class NewsAction extends CommonAction
{
    ...
    public function create()
    {
        if ($this->isPost()) {
            ...
            $obj = D('Communitynews');
            if ($obj->add($data)) {
                $this->tuSuccess('物业通知发布成功', U('news/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    public function checkCreate()
    {
        ...
        if ($words = D('Sensitive')->checkWords($data['title'])) {
            $this->tuError('物业通知标题含有敏感词：' . $words);
        }
        $data['intro'] = htmlspecialchars($data['intro']);
        if (empty($data['intro'])) {
            $this->tuError('物业通知简介不能为空');
        }
        if ($words = D('Sensitive')->checkWords($data['intro'])) {
            $this->tuError('物业通知简介含有敏感词：' . $words);
        }
        $data['details'] = htmlspecialchars($data['details']);
        if (empty($data['details'])) {
            $this->tuError('物业通知内容不能为空');
        }
        if ($words = D('Sensitive')->checkWords($data['details'])) {
            $this->tuError('物业通知内容含有敏感词：' . $words);
        }
    ...
    public function edit($news_id)
    {
        ...
        if ($this->isPost()) {
            ...
            if (false !== $obj->save($data)) {
                $this->tuSuccess('操作成功', U('news/index'));
            }
            $this->tuError('操作失败');
        } else {
