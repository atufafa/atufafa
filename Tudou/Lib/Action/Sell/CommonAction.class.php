...
class CommonAction extends Action
{
    ...
    protected function _initialize(){
        ...
        if(strtolower(MODULE_NAME) != 'login' && strtolower(MODULE_NAME) != 'public') {
           ...
           if($_GET['type']==1){
               $this->shop = M('lifes_authentication')->where(array('user_id' => $this->uid,'close' => 0,'examine'=>1))->find();
               if(empty($this->shop)){
                   $this->error('该用户没有开通信息管理', U('login/index/type/1'));
               }
               session('shop',$this->shop);
               $this->assign('SHOP',$this->shop);
           }elseif($_GET['type']==2){
               $this->shop = M('lifes_vehicle')->where(array('user_id' => $this->uid,'close' => 0,'examine'=>1))->find();
               if(empty($this->shop)){
                   $this->error('该用户没有开通信息管理', U('login/index/type/2'));
               }
               session('shop',$this->shop);
               $this->assign('SHOP',$this->shop);
           }
    ...
    private function parseTemplate($template = ''){
        ...
        define('NOW_PATH', BASE_PATH . '/themes/' . $theme . 'Sell/');
        define('THEME_PATH', BASE_PATH . '/themes/default/Sell/');
        define('APP_TMPL_PATH', __ROOT__ . '/themes/default/Sell/');
        if ('' == $template) {
