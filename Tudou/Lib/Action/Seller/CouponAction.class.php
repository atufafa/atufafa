...
class CouponAction extends CommonAction
{
    ...
    public function create()
    {
        if ($this->isPost()) {
            ...
            $obj = D('Coupon');
            if ($obj->add($data)) {
                $this->tuMsg('添加成功，请等待网站管理员审核', U('coupon/sale'));
            }
            $this->tuMsg('操作失败');
        } else {
            $this->display();
    ...
    private function createCheck()
    {
        ...
        if (!isDate($data['expire_date'])) {
            $this->tuMsg('过期日期格式不正确');
        }
        $data['intro'] = htmlspecialchars($data['intro']);
        if (empty($data['intro'])) {
            $this->tuMsg('优惠券描述不能为空');
        }
		$data['full_price'] = (float) ($data['full_price']);
        if (empty($data['full_price'])) {
            $this->tuError('满多少钱不能为空');
        }
        $data['reduce_price'] = (float) ($data['reduce_price']);
