...
class IndexAction extends CommonAction{
    public function index(){
        ...
        $this->assign('shops',D('Shop')->where(array('shop_id'=>$this->shop_id,'user_id'=>$this->uid))->find());
    ...
    public function pay(){
        $price = I('money','','trim');
        $money=$this->_CONFIG['cash']['user_capitalpay'];
        if($price<$money){
            $this->ajaxReturn(array('code'=>'0','msg'=>'充值金额必须大于'.$money.'元'));
        }
    ...
    public function pay2(){
        $price = I('money','','trim');
        $money=$this->_CONFIG['cash']['return_recharge_money'];
        if($price<$money){
            $this->ajaxReturn(array('code'=>'0','msg'=>'充值金额必须大于'.$money.'元'));
        }
        $code = I('code','','trim,htmlspecialchars');
