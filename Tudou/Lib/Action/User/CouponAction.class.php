...
class CouponAction extends CommonAction{
    ...
    public function couponloading(){
        ...
        // print_r($Coupondownload);die;
        $Coupondownload = D('Coupondownload')->itemsByIds($Coupondownload);
    ...
    public function coupondel($download_id){
        ...
        D('Coupondownload')->where(array('download_id'=>$download_id))->save(array('softdel'=>1));
        ...
        if ($this->isPost()) {
			...
			if(!empty($user['mobile'])){
				if (FALSE !== D('Coupondownload')->save(array('download_id' => $download_id, 'user_id' => $user['user_id']))){
                    ...
                    $this->tuMsg('恭喜您赠送成功', U('coupon/index'));
                }else{
                    $this->tuMsg('操作失败');
                }
