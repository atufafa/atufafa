...
class CrowdAction extends CommonAction {
   ...
   //往期众筹
	...
	public function create() {
        if ($this->isPost()) {
            ...
            if ($goods_id = $obj->add($data)) {
                ...
                $this->tuMsg('恭喜你添加众筹成功', U('crowd/crowd_list'));
            }
            $this->tuMsg('操作失败');
        } else {
            $this->display();
    ...
    private function createCheck() {
		...
		$data['all_price'] = (float) ($data['all_price']);
        ...
        if ($goods_id = (int) $goods_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    ...
                    $this->tuMsg('恭喜您编辑众筹成功', U('crowd/crowd_list'));
                }
                $this->tuMsg('操作失败');
            } else {
		...
		if ($goods_id = (int) $goods_id) {
			...
			if ($data = $this->_post('data', false)) {
				if (!empty($data['price']) && !empty($data['content']) && !empty($data['max_num']) && !empty($data['fahuo'])){
					...
					$this->tuMsg('操作成功', U('crowd/setting',array('goods_id'=>$detail['goods_id'])));
				}else{
					 $this->tuMsg('内容不能为空');
				}
			}else{
	...
	public function type_edit($type_id)
	{
		if ($type_id = (int) $type_id) {
            ...
            if (!$detail = $Crowd->find($goods_id)) {
                $this->tuMsg('请选择要设置的众筹');
            }
            if ($detail['user_id'] != $this->uid) {
                $this->tuMsg('请不要操作别人的众筹');
            }
            if ($detail['closed'] != 0) {
                $this->tuMsg('该众筹已被删除');
            }
		...
		if ($goods_id = (int) $goods_id) {
			...
			if ($data = $this->_post('data', false)) {
				if (!empty($data['content'])){
					...
					$this->tuMsg('操作成功', U('crowd/project',array('goods_id'=>$detail['goods_id'])));
				}else{
					 $this->tuMsg('内容不能为空');
				}
			}else{
		...
		if ($ask_id = (int) $ask_id) {
			...
			if ($data = $this->_post('data', false)) {
				if (!empty($data['answer_c'])){
					...
					$this->tuMsg('操作成功', U('crowd/ask',array('goods_id'=>$ask['goods_id'])));
				}else{
					 $this->tuMsg('内容不能为空');
				}
			}else{
   ...
   //中奖状态
	public function zhong($list_id){
		if(!empty($list_id)){
			...
			if($Crowdlist->where(array('list_id'=>$list_id))->save($data)){
				$this->tuMsg('设为中奖成功', U('crowd/lists',array('goods_id'=>$list['goods_id'])));
			}
		}else{
			$this->tuMsg('错误');		
		}
    ...
    public function used() {
        if ($this->isPost()) {
			if (empty($this->uid)) {
            	$this->tuMsg('登录状态失效!', U('Wap/passport/login'));
       		}
                ...
                if (!empty($var)) {
                    ...
                    if (!empty($data) && $data['uid'] == $this->uid && (int) $data['is_check'] == 0 && (int) $data['status'] == 1) {
                        if ($obj->save(array('order_id' => $data['order_id'], 'is_check' => 1))) {        
							if (!empty($data['price'])) {
								//这里暂时没增加逻辑
								$this->tuMsg('众筹券' . $order_id . '验证成功！', $crowd_list_jump_url);
							} else {
								 $this->tuMsg('验证失败', $crowd_list_jump_url);
							}
                        }
                    } else {
                        $this->tuMsg('该众筹券无效或已经使用', $crowd_list_jump_url);
                    }
                }
            }
        } else {
