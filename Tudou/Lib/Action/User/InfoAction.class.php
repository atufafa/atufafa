...
class InfoAction extends CommonAction{
    ...
    public function nickname(){
        if ($this->isPost()) {
            ...
            $this->tuMsg('昵称已经更新', U('info/nickname'));
        }
        $this->display();
    }
	...
	public function carbrand(){
		 if($this->isPost()){
            $carBrand_id = I('carBrand_id');
            if(empty($carBrand_id)){
                $this->ajaxReturn(array('code' => '0', 'msg' => '没有选择品牌'));
            }
            if(D('Users')->save(array('user_id' => $this->uid, 'carBrand_id' => $carBrand_id))){
				$this->ajaxReturn(array('code' => '1', 'msg' => '操作成功','url'=>U('info/carbrand')));
            }else{
				$this->ajaxReturn(array('code' => '0', 'msg' => '操作失败'));
			}
        }else{
       	 	$this->assign('carbrands',$carbrands = D('CarBrand')->select());
            $this->display();
        }
    }
    ...
    public function account(){
        if ($this->isPost()) {
            ...
            D('Passport')->logout();
            $this->tuMsg('您的帐号已经更新', U('Wap/index/index'));
        }
        $this->display();
    }
	//设置，编辑支付密码
