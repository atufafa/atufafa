...
class AllAction extends CommonAction {
    ...
    public function load(){
        ...
        $where_market = array('audit'=>1,'closed' => 0);//菜市场
        $where_store = array('audit'=>1,'closed' => 0);//便利店
        $where_jifen = array('closed' => 0, 'audit' => 1,'end_date' => array('EGT', TODAY));
        $where_linyuan = array('closed' => 0, 'audit' => 1,'end_date' => array('EGT', TODAY));
        ...
        if ($keyword) {
            ...
            $where_market['product_name'] = array('LIKE', '%' . $keyword . '%');//菜市场
            $where_store['product_name'] = array('LIKE', '%' . $keyword . '%');//便利店
            $where_jifen['title'] = array('LIKE', '%' . $keyword . '%');
            $where_linyuan['title'] = array('LIKE', '%' . $keyword . '%');
		...
		$Tuan=D('Tuan');
		$Village = D('Village');
		$market = D('Marketproduct');
		$store = D('Storeproduct');
        $jifen = D('Integralgoodslist');
        $zero = D('Pindan');
		...
		$list_appoint=$Appoint->Field('"家政" as t_name,"appoint/detail" as t_url,"appoint_id" as t_param,appoint_id as t_id,title as t_title,photo as t_photo,concat("￥",round(price,2)) as t_note')->order($orderby)->where($where_appoint)->select();
		...
		$list_booking=$Booking->Field('"订座" as t_name,"booking/detail" as t_url,"shop_id" as t_param,shop_id as t_id,shop_name as t_title,photo as t_photo,concat("￥",round(price,2)) as t_note')->order($orderby)->where($where_booking)->select();
		...
		$list_crowd=$Crowd->Field('"众筹" as t_name,"crowd/detail" as t_url,"goods_id" as t_param,goods_id as t_id,title as t_title,photo as t_photo,concat("￥",round(all_price,2)) as t_note')->order($orderby)->where($where_crowd)->select();
                $list_ele=$Ele->Field('"外卖" as t_name,"ele/index" as t_url,product_name as t_title,photo as t_photo,concat("￥",round(settlement_price,2)) as t_note')->order($orderby)->where($where_ele)->select();
        ...
        $list_market=$market->Field('"菜市场" as t_name,"market/index" as t_url,product_name as t_title,photo as t_photo,concat("￥",round(price,2)) as t_note')->order($orderby)->where($where_market)->select();
        $list_store=$store->Field('"便利店" as t_name,"store/index" as t_url,product_name as t_title,photo as t_photo,concat("￥",round(price,2)) as t_note')->order($orderby)->where($where_store)->select();
        $list_jifen=$jifen->Field('"积分商城" as t_name,"jifen/detail2" as t_url,"goods_id" as t_param,goods_id as t_id,title as t_title,photo as t_photo,concat("￥",round(mall_price,2)) as t_note')->order($orderby)->where($where_jifen)->select();
        $list_zero=$zero->Field('"0元免费拿" as t_name,"tuan/tuan_detail" as t_url,"tuan_id" as t_param,tuan_id as t_id,title as t_title,photo as t_photo,concat("￥",round(mall_price,2)) as t_note')->order($orderby)->where($where_linyuan)->select();

		$list_goods=$Goods->Field('"商城" as t_name,"mall/detail" as t_url,"goods_id" as t_param,goods_id as t_id,title as t_title,photo as t_photo,concat("￥",round(mall_price,2)) as t_note')->order($orderby)->where($where_goods)->where(['huodong'=>0])->select();
		$list_hotel=$Hotel->Field('"酒店" as t_name,"hotels/detail" as t_url,"hotel_id" as t_param,hotel_id as t_id,hotel_name as t_title,photo as t_photo,concat("￥",round(price,2)) as t_note')->order($orderby)->where($where_hotel)->select();
		...
		$list_tuan=$Tuan->Field('"抢购" as t_name,"tuan/detail" as t_url,"tuan_id" as t_param,tuan_id as t_id,title as t_title,photo as t_photo,concat("￥",round(tuan_price,2)) as t_note')->order($orderby)->where($where_tuan)->select();
        ...
        if(!empty($list_market)){
            $list=array_merge($list,$list_market);
        }
        if(!empty($list_store)){
            $list=array_merge($list,$list_store);
        }
        ...
        if (!empty($config['operation']['jifen'])){
            if(!empty($list_jifen)){
                $list=array_merge($list,$list_jifen);
            }
        }
        if(!empty($list_zero)){
            $list=array_merge($list,$list_zero);
        }
