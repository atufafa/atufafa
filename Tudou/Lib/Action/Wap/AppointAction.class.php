...
class AppointAction extends CommonAction {
    ...
    public function detail($appoint_id) {
        ...
        $pingnum = $Appointdianping->where(array('appoint_id' => $appoint_id,'closed'=>0,'show_date' => array('ELT', TODAY)))->count();
    ...
    public function create($appoint_id){
        ...
        //查询用户是否领取了优惠劵
        $coupon=D('Coupondownload')->where(['shop_id'=>$appoint_shop['shop_id'],'user_id'=>$this->uid,'is_used'=>0])->find();
        if(!empty($coupon) &&  $cate_id['price'] >= $coupon['full_price']){
            $ypuhui=D('Coupon')->where(['coupon_id'=>$coupon['coupon_id']])->find();
            $full_price=$ypuhui['reduce_price'];
            $coupon_id=$ypuhui['coupon_id'];
            $need_pay=$cate_id['price']-$full_price;
        }else{
            $full_price=0;
            $coupon_id=0;
            $need_pay= $cate_id['price'];
        }
    ...
    public function pay(){
        ...
        $this->assign('order',$order);
        $this->assign('useEnvelope',D('Appointorder')->GetuseEnvelope($this->uid,$order['shop_id']));
        $this->display();
    }
    ...
    public function pay2($type = '0'){
        ...
        if($card_id){
            ...
            if($order['zhi_money'] > $res['cardMoney']){
                $price = $order['need_pay'] - $res['cardMoney'];//实际支付 = 家政金额 - 兑换码金额
                D('Appointorder')->where(array('order_id'=>$order_id))->save(array('card_id'=>$card_id,'cardNumber'=>$res['cardNumber'],'cardMoney'=>$res['cardMoney'],'zhi_money'=>$price));
            }
        ...
        if($price){
            ...
            $need_pay = $order['zhi_money'];//实际支付 = 家政金额
        ...
        if($this->ispost()) {
            ...
            D('Appointorder')->where(array('order_id' => $order_id))
                ->save(array('zhi_money' => $money, 'envelope_id' => $hongbao_id,'envelope_money'=>$hongbao_money));
//            //从会员中心过来付款
//            $type = (int)$this->_get('type');
//            if ($type == 2) {
//                //取消优惠卡
//                $mm=D('Appointorder')->where(array('order_id' => $order_id))->save(array('card_id' => '', 'cardNumber' => '', 'cardMoney' => '', 'need_pay' => $order['need_pay'] + $order['cardMoney']));
