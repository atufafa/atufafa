...
class CommunityAction extends CommonAction{
    ...
    public function owner($community_id){
        ...
        if ($this->isPost()) {
            ...
            if (!($res = $obj->where(array('community_id' => $community_id, 'user_id' => $this->uid))->find())) {
                if ($obj->add($data)) {
                    $this->tuMsg('申请成功，请等待物业审核', U('community/detail', array('community_id' => $community_id)));
                }
                $this->tuMsg('申请失败');
            }
            $this->tuMsg('不能重复申请');
    ...
    public function post(){
        ...
        if ($this->isPost()) {
            ...
            $last = $obj->add($data);
            if ($last) {
                $this->tuMsg('帖子发布成功', U('community/tie', array('post_id' => $last)));
            }
            $this->tuMsg('发帖失败');
        } else {
            $this->assign('detail', $detail);
    ...
    public function reply($post_id){
        ...
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                ...
                $this->tuMsg('帖子发布成功', U('community/tie', array('post_id' => $post_id)));
            }
            $this->tuMsg('回帖失败');
        } else {
            $this->assign('tie', $tie);
    ...
    public function feedback($community_id){
        ...
        if ($this->isPost()) {
            ...
            $obj = D('Feedback');
            if ($obj->add($data)) {
                $this->tuMsg('反馈提交成功', U('community/detail', array('community_id' => $community_id)));
            }
            $this->tuMsg('操作失败');
        } else {
            $this->assign('detail', $detail);
    ...
    public function orderpay(){
        if(empty($this->uid)){
            $this->ajaxReturn(array('code' => 'login'));
        }
        if(IS_AJAX){
            $community_id = (int) $_POST['community_id'];
            if (empty($community_id)) {
                $this->ajaxReturn(array('code' => '0', 'msg' => '小区不存在'));
            }
            ...
            if ($res === $type) {
                ...
                if($total2 == $total){
					...
					$logs = array(
						'type' => 'community', 
						'user_id' => $this->uid, 
						'order_id' => $detail['order_id'].'-'.time(),
						'order_ids' => $detail['order_id'].'-'.time(),  
						'code' => $code, 
						'types' => $pp3, 
						'need_pay' => $total,
					...
					if($logs['log_id'] = D('Paymentlogs')->add($logs)){
						if(!$res = D('Communityorder')->where(array('community_id' => $community_id,'order_id' => $detail['order_id']))->save(array('order_ids'=>$detail['order_id'].'-'.time()))){
							$this->ajaxReturn(array('code' => '0','msg' => '非法操作稍后重试'));
						}
						$this->ajaxReturn(array('code' => '1','msg' => '生成订单成功，正在为您跳转支付', 'url' => U('payment/payment', array('log_id' => $logs['log_id']))));
					}else{
						$this->ajaxReturn(array('code' => '0','msg' => '生成订单失败'));
					}
                }else{
                    $this->ajaxReturn(array('code' => '0','msg' => '金额不正确，或者单选缴费项目试试'));
                }
            } else {
                $this->ajaxReturn(array('code' => '0','msg' => '账单不合法'));
            }
        }
    }
}