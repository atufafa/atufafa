...
class MallAction extends CommonAction{
    ...
    public function index(){
        ...
        //天天特价
        $tejia=D('MallactivityGoods')->where(['type_id'=>1,'colsed'=>0,'audit'=>1,'type'=>1])->select();
        foreach ($tejia as $val){
            $goods_ids=$val['goods_id'];
        }
        $this->assign('elespecial',$tejia);
        $this->assign('tegoods',D('Goods')->itemsByIds($goods_ids));
        //限时秒杀
        ...
        $this->assign('miaogoods',D('Goods')->itemsByIds($good_id));
    ...
    public function loaddata(){
        ...
        switch($order){
			...
			case '5':
                ...
                break;
			default:
                $orderby = array('top_time' => 'desc','check_price' =>'desc','mall_price'=>'asc');
                break;
        }
    ...
    //拼单
    public function pindan($goods_id){
        $goods_id = (int) $goods_id;
        if (empty($goods_id)) {
            $this->ajaxReturn(array('status' => 'error', 'msg' => '请选择产品'));
        }
    ...
    public function order_pin(){
            ...
            foreach ($goods as $k => $v) {
                foreach($v as $key => $val){
                    ...
                    if(!empty($goods[$k][$key][sky])){
                        ...
                        $goods[$k][$key]['ping_money']=$spt['pin_price'];
            ...
            foreach ($goods as $val) {
                ...
                $price = $val['ping_money'] * $num[$val['goods_id']];
                var_dump($val['ping_money'], $num[$val['goods_id']]);
...
var_dump($price);die;
   ...
   public function get_spec($goods_id){
	...
	// $sql = "SELECT a.name,a.order,b.* FROM __PREFIX__tp_spec AS a INNER JOIN __PREFIX__tp_spec_item AS b ON a.id = b.spec_id WHERE b.id IN($keys) ORDER BY a.order";
            ...
            $sql  = "SELECT a.name,a.order,b.* FROM tu_tp_spec AS a INNER JOIN tu_tp_spec_item AS b ON a.id = b.spec_id WHERE b.id IN($keys) ORDER BY a.order";
    ...
    public function paycode2(){
        ...
        if ($code == 'wait') {
            ...
            foreach($redbag as $key=>$value){
                if($value['reduce_price']>0){
                    $data['log_id']=$log_id;
                    $data['user_id']=$this->uid;
                    $data['shop_id']=$key;
                    $data['reduce_price']=$value['reduce_price'];
                    $data['use_id']=$value['use_id'];
                    $data['type']=$value['type'];
                    $data['create_time']=date('Y-m-d H:i:s',time());
                    M('shop_pay_log')->add($data);
                }
            }
    ...
    public function pay2(){
        ...
        $envelope=D('UserEnvelope')->where(array('user_envelope_id'=>$honbao['envelopes_id']))->find();
        //var_dump($envelope);
        if(!empty($envelope['shop_id'])){
            D('UserEnvelope')->where(array('user_id'=>$this->uid,'user_envelope_id'=>$honbao['envelopes_id']))->save(array('is_use'=>1));
            $arr=array(
                'user_id'=>$this->uid,
           ...
           // echo 2;
            $ss =$envelope['num']-1;
            D('UserEnvelope')->where(array('user_id'=>$this->uid,'user_envelope_id'=>$honbao['envelopes_id']))->save(array('num'=>$ss));
            $cha=D('UserEnvelope')->where(array('user_id'=>$this->uid,'user_envelope_id'=>$honbao['envelopes_id']))->find();
