...
class NewsAction extends CommonAction {
	 ...
	 public function load(){
        ...
        switch ($order) {
            case '1':
                ...
                break;
            case '2':
                $orderby = array('zan' => 'desc');
                $linkArr['order'] = $order;
                break;
            case '3':
                $orderby = array('donate_num' => 'desc');
                $linkArr['order'] = $order;
                break;
            case '4':
                ...
                break;
            default:
                $orderby = array('create_time' => 'desc', 'article_id' => 'desc', 'istop' => 'desc');
                break;
        }
		$this->assign('order', $order);
		...
		foreach ($list as $k => $val) {
            ...
            $list[$k]['count'] = $count;
			$list[$k]['comment'] = D('Articlecomment')->where(array('post_id'=>$val['article_id'],'parent_id'=>0))->count();
			$list[$k]['cate'] = D('Articlecate')->where(array('cate_id'=>$val['cate_id']))->getField('cate_name');
			$list[$k]['shop'] = D('Shop')->where(array('shop_id'=>$val['shop_id']))->find();
        }
	 ...
	 public function loaddata(){
        ...
        switch($order){
            case 2:
                ...
                break;
            default:
                $orderby = array('create_time' => 'desc', 'istop' => 'desc');
                break;
        }
	...
	public function cai(){
		...
		if(empty($cai)){
			...
			$this->tuMsg('踩成功',U('news/detail', array('article_id' => $detail['post_id'])));
		}else{
			$this->tuMsg('不能踩自己');	
		}
    }
	...
	public function zan(){
	    ...
	    $this->tuMsg('点赞成功',U('news/detail', array('article_id' => $detail['article_id'])));
    }
	...
	public function donate(){
		...
		$money = (I('money', 0, 'trim,htmlspecialchars'));
        $detail = D('Article')->find($article_id);
        if(empty($detail)){
