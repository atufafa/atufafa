...
class RoomAction extends CommonAction{
     ...
     private $authentication=array('end_time','user_id','handphoto','positivephoto','recommend','backphoto','businessphoto','personname','persontel','address','type','deposit','management');
    ...
    public function _initialize(){
        ...
        $this->lifechannel = M('LifeClass')->where(array('channel_id'=>1))->select();
        $this->assign('lifecate', $this->lifecate);
        $this->assign('channel', $this->lifechannel);
        //判断类型为房的
        $this->cates = M('LifeClass')->where(array('channel_id'=>1))->select();
        $this->assign('cates', $this->cates);
        //判断类型为1的
        $this->cate_name=M('LifeClass')->where(array('channel_id'=>1,'cate_id'=>$this->_param('cat')))->getField('cate_name');
	...
	public function index(){
		...
		if($cate_id = (int) $this->_param('cat')){
             $cate=M('LifeClass')->where(array('channel_id'=>1,'cate_id'=>$cate_id))->find();
            if($cate){
                $map['cate_id'] = $cate_id;
        ...
        $linkArr['p'] = '0000';
        $this->assign('nextpage', U('room/load', $linkArr));
    ...
    public function recognize(){
        $users_id=$this->uid;
            ...
            $this->assign('money',$config['site']['room_money']);
            $this->assign('moneys',$config['site']['room_moneys']);
            $detail = D('Lifesauthentication')->where(array('user_id'=>$users_id))->find();
            //var_dump($detail);die;
    ...
    public function renz(){
        ...
        $data = $this->checkFields($this->_post('data', false), $this->authentication);
       ...
       if(D('Lifesauthentication')->add($data)){
            $log_id = D('Paymentlogs')->add($logs);
            $this->ajaxReturn(array('code' => '1', 'msg' => '正在跳转到支付页面', 'url' => U('room/psy', array('log_id' => $log_id))));
        }
    }
    ...
    public function yuyue(){
       ...
       $data['time']=$this->_post('time','htmlspecialchars');
        ...
        $exp=D('Lifereserve')->where(['user_id'=>$this->uid,'life_id'=>$data['life_id'],'type'=>1,'is_pay'=>1,'close'=>0])->find();
        if(!empty($exp)){
            $this->tuMsg('您已预约过该商家，请前往个人中心预约列表查看',U('room/details',array('life_id'=>$data['life_id'])));
        }
       if(empty($data['time'])){
            $this->tuMsg('预约时间为空');
       }
       ...
       $obj = D('Lifereserve');
        if ($obj->add($data)){
              $this->tuMsg('预约成功！,请前往个人中心，预约信息列表查看',U('room/index/1'));
            }else{
   ...
   public function detail($life_id){
        ...
        D('Lifes')->updateCount($life_id, 'views');
        $this->assign('cate', $cate);
        ...
        $this->assign('scroll', $scroll = D('Lifes')->getScroll());//首页循环播放
        //购买返利卷
        $this->assign('payment', D('Payment')->getPayments(true));
   ...
   public function details($life_id){
    ...
    public function fabu(){
        ...
        //判断类型等于二手、车辆、房屋、培训、招聘、服务、兼职
            $users_life=D('Lifesauthentication')->where(array('user_id'=>$this->uid))->find();

            $vehicle=D('Lifesvehicle')->where(array('user_id'=>$this->uid))->find();

            if(!empty($vehicle['user_id'])){
               $this->error('该账号已进行卖车公司认证了，如有需要请换一个账号进行卖房公司认证');
               
             }

            if($users_life['examine']!=1){
               $this->error('未通过公司认证，请认证过后再次发布',U('room/recognize'));
            }
        ...
        if($this->isPost()){
            ...
            if($life_id = D('Lifes')->add($data)){
                
                //传图
                $photo = $this->_post('photo', false);
                if($photo){
                    D('Lifes')->where(array('life_id'=>$life_id))->save(array('photo'=>$photo['0']));
                }
                ...
                if($money > 0){
                    ...
                    if($this->member['money'] >= $money){
                        ...
                        D('Lifes')->save(array('life_id'=>$life_id,'price'=>$money,'status'=>1));//改变订单状态
                        $this->ajaxReturn(array('code'=>'1','msg'=>'发布信息成功扣费【'.round($money,2).'】元','url'=>U('wap/room/index')));
                    }else{
                    ...
                    //后台没配置分类需要扣费
                    $this->ajaxReturn(array('code'=>'1','msg'=>'发布信息成功，通过审核后将会显示','url'=>U('wap/room/index'))); 
                }
            }
  ...
  //前台获取商圈
    ...
    public function report($life_id){
        ...
        if($this->isPost()){
            ...
            $shop_id=session('fanli');
            $data['shop_id']=$shop_id['user_id'];
            ...
            if(false !== D('Lifereport')->add($data)) {
               $this->ajaxReturn(array('code'=>'1','msg'=>'举报成功','url'=>U('room/index')));  
            }
    ...
    //查询
     public function search(){
        ...
        $this->assign('nextpage', LinkTo('room/searchload', array('keyword' => $keyword, 'cat' => $cat, 'order' => $order, 't' => NOW_TIME, 'p' => '0000')));
        $this->display();
    }
    //加载查询
