...
class ThreadAction extends CommonAction {
    ...
    public function collect() {
        if (empty($this->uid)) {
            $this->tuMsg('请先登录后操作！',U('passport/login'));
        }
        ...
        if (D('Threadcollect')->check($thread_id, $this->uid)) {
            if(D('Threadcollect')->where(array('thread_id'=>$thread_id,'user_id'=>$this->uid))->delete()){
                ...
                $this->tuMsg('取消关注成功！',U('thread/lists',array('cate_id'=>$detail['cate_id'])));
            }
            $this->tuMsg('取消失败');
        }else{
            $data = array(
            ...
            if (D('Threadcollect')->add($data)) {
                ...
                $this->tuMsg('恭喜您关注成功！',U('thread/detail',array('thread_id'=>$thread_id)));
            }
            $this->tuMsg('关注失败');
        }
    }
        ...
        if(!$thread_id = (int)$thread_id){
            ...
            if($this->isPost()){
                ...
                if ($words = D('Sensitive')->checkWords($data['title'])) {
					$this->ajaxReturn(array('code'=>'0','msg'=>'标题含有敏感词：' . $words));
                } 
                $data['details'] = htmlspecialchars($this->_param('details'));
                if(empty($data['details'])){
					$this->ajaxReturn(array('code'=>'0','msg'=>'详情不能为空'));
                }
                if ($words2 = D('Sensitive')->checkWords($data['details'])) {
					$this->ajaxReturn(array('code'=>'0','msg'=>'详情含有敏感词：' . $words2));
                } 
                ...
                if($post_id = D('Threadpost')->add($data)){
					...
					$this->ajaxReturn(array('code'=>'1','msg'=>'恭喜您发帖成功','url'=>U('thread/detail',array('thread_id'=>$thread_id))));
                }else{
        ...
        if(!$post_id = (int)$post_id){
            ...
            if ($this->isPost()) {
                    ...
                    if($cid = D('Threadpostcomments')->add($data)){
						...
						$this->ajaxReturn(array('code'=>'1','msg'=>'回复成功','url'=>U('thread/postdetail',array('post_id'=>$post_id))));
                    }else{
