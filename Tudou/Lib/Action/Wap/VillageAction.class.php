...
class VillageAction extends CommonAction{
    ...
    public function detail($village_id){
		...
		$this->assign('tuan', $tuan =D('VillageEnter')->get_enter($this->city_id,$type = 2,$village_id,$limit = 10));
    ...
    public function tieba()
    {
		...
		$this->assign('reply2', $reply2 = D('Villagebbsreplys')->where(array('village_id' => $community_id, 'audit' => 1,'closed'=>0))->count());
        $this->assign('count', $count2 = D('Village_bbs')->where(array('village_id' => $community_id, 'audit' => 1,'closed'=>0))->count());
        $this->display();
    }
    //贴吧帖子加载
    public function loadtieba(){
        ...
        $Tieba = D('Village_bbs');
        import('ORG.Util.Page');
        $map = array('village_id' => $community_id, 'audit' => 1,'closed'=>0);
    ...
    public function post()
    {
        ...
        if($users['is_aux'] !=1){
            $this->error('未实名认证，请实名认证后才能发帖！',U('user/usersaux/index'));
        }
        ...
        if ($this->isPost()) {
            ...
            $last = $obj->add($data);
            if ($last) {
                $this->tuMsg('帖子发布成功', U('village/tie', array('post_id' => $last)));
            }
            $this->tuMsg('发帖失败');
        } else {
            $this->assign('detail', $detail);
    ...
    public function reply($post_id)
    {
        ...
        $users =D('Users')->where(['user_id'=>$this->uid])->find();
        if($users['is_aux'] !=1){
            $this->error('未实名认证，请实名认证后才能回帖！',U('user/usersaux/index'));
        }
        ...
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $this->tuMsg('回帖成功', U('Village/tie', array('post_id' => $post_id)));
            }
            $this->tuMsg('回帖失败');
        } else {
            $this->assign('tie', $tie);
    ...
    public function suggestion($village_id)
    {
        ...
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $this->tuMsg('意见反馈提交成功', U('village/detail', array('village_id' => $village_id)));
            }
            $this->tuMsg('操作失败');
        } else {
            $this->assign('detail', $detail);
    ...
    public function loadworkers()
    {
        ...
        $village=D('Village')->where(['village_id'=>$community_id])->find();
        $user=D('Users')->where(['user_id'=>$village['user_id']])->find();
        ...
        if ($data = $this->_post('data', false)) {
            ...
            if ($phone_id = D('Convenientphone')->add($data)) {
                if (D('Convenientphonemaps')->add(array('phone_id' => $phone_id, 'community_id' => $community_id))) {
                    $this->tuMsg('您的申请提交成功,等待审核', U('community/detail', array('community_id' => $community_id)));
                } else {
                    $this->tuMsg('申请失败');
                }
            } else {
    ...
    public function joinvillage($village_id)
    {
	 ...
	 // 从村邻居
    public function neighbor($village_id){
        $village_id = (int) $village_id;
        $obj = D('Village');
        if (empty($village_id)) {
            $this->error('没有该乡村');
        ...
        $this->assign('detail', $detail);
        $this->assign('nextpage', LinkTo('village/loadneighbor', array('village_id' => $detail['village_id'], 't' => NOW_TIME, 'p' => '0000')));
        $this->display();
    }
    //乡村居民加载
    public function loadneighbor(){
        ...
        if(!($detail = $Village->find($community_id))) {
            $this->error('没有该乡村');
            die;
        }
        if($detail['closed']) {
        ...
        $obj = D('Villagejoin');
        import('ORG.Util.Page');
        $map = array('village_id' => $village_id);
    ...
    public function loadtuan($village_id = 0){
		$village_id = (int) $this->_param('village_id');
		$query = array('village_id' => $village_id,'closed' => 0, 'audit' => 1, 'type' => 2);
		$lists = D('VillageEnter')->where($query)->order(array('enter_id' => 'desc'))->select();
		...
		import('ORG.Util.Page');
		$obj = D('Tuan');
		$map = array('closed' => 0, 'audit' => 1, 'expire' => array('EGT', TODAY), 'tuan_id' => array('in',$tuan_ids));
        ...
        $list = $obj->where($map)->order($orderby)->select();
    ...
    public function fabu(){
        $village_id = (int) $this->_get('village_id');
        $Village = D('Village');
        if (empty($village_id )) {
            $this->error('没有该小区');
            die;
        }
        if (!($detail = $Village->find($village_id ))) {
            $this->error('没有该小区');
            die;
        }
        if ($detail['closed']) {
        ...
        $count = D('Village_worker')->where(array('village_id' => $village_id, 'user_id' => $this->uid))->count();
        if ($count == 0) {
            $this->error('您没有权限');
        }
        if ($this->isPost()) {
            ...
            $data['village_id'] = $detail['village_id'];
            if($obj->add($data)){
				$this->ajaxReturn(array('code'=>'1','msg'=>'发布成功','url'=>U('village/detail', array('village_id' => $village_id))));
            }
			$this->ajaxReturn(array('code'=>'0','msg'=>'发帖失败'));
        } else {
            $this->assign('detail', $detail);
            $this->display();
        }
    ...
    //乡村发布通知检测
    private function fabuCheck(){
        $data = $this->checkFields($this->_post('data', false), array('type','title', 'context'));
        $data['title'] = htmlspecialchars($data['title']);
        if(empty($data['title'])){
		...
		if(empty($data['type'])){
			$this->ajaxReturn(array('code'=>'0','msg'=>'类型必须选择'));
        }
        $data['context'] = SecurityEditorHtml($data['context']);
        if($words = D('Sensitive')->checkWords($data['context'])) {
