...
class OrderAction extends CommonAction
{
            ...
            if (!($res = $obj->where(array('user_id' => $user_id, 'order_date' => $data['order_date']))->find())) {
                if ($order_id = $obj->add($data)) {
                    foreach ($datas as $k => $val) {
                        D('Communityorderproducts')->add(array('order_id' => $order_id, 'community_id' => $community_id, 'type' => $k, 'money' => $val['money'], 'bg_date' => $val['bg_date'], 'end_date' => $val['end_date']));
                    }
                    $this->tuMsg('添加成功', U('order/index', array('user_id' => $user_id)));
                } else {
                    $this->tuMsg('添加失败');
                }
            }
            $this->tuMsg('该账单已存在', U('order/index', array('user_id' => $user_id)));
        } else {
    ...
    public function edit($order_id)
    {
        ...
        if ($this->isPost()) {
            ...
            $this->tuMsg('修改成功', U('order/index', array('user_id' => $detail['user_id'])));
        } else {
