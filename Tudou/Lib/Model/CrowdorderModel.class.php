		...
		if(!empty($user_id)){
			...
			if(!empty($user_address_default)){
				...
				if(!empty($user_address)){
					...
					if($type == 1){
						$this->tuMsg('您暂无收货地址！,正在为您跳', U('wap/address/addrcat', array('goods_id' => $goods_id,'type'=>crowd))); 
					}else{
						$this->tuJump(U('members/malladdress/create',  array('type' => crowd, 'goods_id' => $goods_id)));
					}
					//return false;//找不到地址
				}
			}
   ...
   //下单时候获取默认地址
    public function addrs_address($order_id,$user_id) {
		 ...
		 if (!empty($order['address_id'])) {
			...
			$this->tuJump(U('members/malladdress/create',  array('type' => crowd, 'goods_id' =>$order['goods_id'])));//都没找到
        }
        return true;//默认
    }
