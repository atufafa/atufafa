<?php
class IntegralgoodslistModel extends CommonModel {

    protected $pk = 'goods_id';
    protected $tableName = 'integral_goodslist';

    //竞价扣款部分逻辑
    public function check_price($goods_id)
    {
        $goods = D('Integralgoodslist')->find($goods_id);
        ...
        if($goods['check_price_money']>0){//如果限额
            ...
            $now_ip = D('IntegralShopJingJiaLog')->where($where)->select();
            $count = count($now_ip);
            //查询IP限制
            $limit = M('Jingjia')->where(['id'=>1])->find();
            if($limit['check_num']<$count){//如果超过限制，不扣费
                //写入浏览数据
                D('IntegralShopJingJiaLog')->add(array(
                    'check_price'=>0,
                ...
                D('Integralgoodslist')->where(['goods_id'=>$goods_id])->save(array('check_num'=>$check_num));
            }else{
                $shop =D('Shop')->where(['shop_id'=>$goods['shop_id']])->find();
                if($shop['bid_money'] <$goods['check_price']){
                    //竞价余额不足，踢出竞价排行
                    D('Integralgoodslist')->where(['goods_id'=>$goods_id])->save(['check_price'=>0,'is_tui'=>0]);
                }
                //扣款操作
                ...
                //写入商品数据
                $check_num = $goods['check_num'] +1 ;
                D('Integralgoodslist')->where(['goods_id'=>$goods_id])->save(array('check_money'=>($goods['check_money'] + $goods['check_price']),'check_num'=>$check_num));
            }
        }
    }
    ...
    public function top_time($goods_id,$check_price,$check_price_money,$end_time){
        ...
        if($check_price){
            if(($shop['bid_money'])<$check_price){
                ...
                D('Integralgoodslist')->where(['goods_id'=>$goods_id])->save(['check_price'=>$check_price,'check_price_money'=>$check_price_money,'start_time'=>NOW_TIME,'end_time'=>NOW_TIME+86400,'is_tui'=>1]);
                return true;
            }
        }else{
            $this->error = '请输入竞价';
