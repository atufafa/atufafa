<?php
class IntegralordergoodsModel extends CommonModel
{
    protected $pk = 'id';
    protected $tableName = 'integral_order_goods';
    ...
    //积分商城合并付款新运费，跟单个商品付款不重复
    public function merge_update_jifen_price($uid,$type,$log_id,$address_id) {
        $log_id = (int)$log_id;
        ...
        if (!empty($Paymentlogs['order_ids'])) {
            ...
            $Ordergoods = D('Integralordergoods');
            ...
            if (empty($ordergoods_list)) {
                ...
                //更新订单物流ID
                foreach($order_ids as $v){
                    D('Integralorder')->save(array('address_id' => $address_id), array('where' => array('order_id' => $v)));
                }
                ...
                foreach ($ordergoods_list as $k => $v) {
                    ...
                    $express_price = $this->replace_add_jifen_price($uid,$address_id,$v['num'],$v['goods_id']);//现在的运费
                    ...
                    D('Integralorder')->save(array('express_price' =>$express_price,'address_id' => $address_id), array('where' => array('order_id' =>$v['order_id'])));
    ...
    //更换地址更新商品运费
    public function replace_add_jifen_price($uid,$id,$num,$goods_id){

        $Paddress = D('Paddress')->where(array('user_id' => $uid, 'id' => $id))->find();
        $detail = D('Integralgoodslist')->where(array('goods_id'=>$goods_id))->find();
    ...
    public function update_jifen_price($uid,$type,$order_id,$address_id) {
        ...
        if (empty($ordergoods_list) || $order['status'] != 0 || $order['user_id'] != $this->uid) {
            ...
            //更新订单物流ID
            D('Integralorder')->save(array('address_id' => $address_id), array('where' => array('order_id' => $order_id)));
            foreach ($ordergoods_list as $k => $val) {
                $Ordergoods->save(array('kuaidi_id' => $address_id), array('where' => array('id' => $val['id'])));
            }
            //更新运费
            foreach ($ordergoods_list as $k => $v) {
                $replace_order_express_price = $this->replace_add_jifen_price($uid,$address_id,$v['num'],$v['goods_id']);
            ...
            //更新总表运费
            D('Integralorder')->save(array('express_price' => $total_express_price,'address_id' => $address_id), array('where' => array('order_id' => $order_id)));
        }
        return true;
    }

    //第一次更新商品运费
    public function calculation_jifen_price($uid,$kuaidi_id,$num,$goods_id,$pc_order) {
        $obj = D('Paddress');
        ...
        return $this->replace_add_jifen_price($uid,$detail['id'],$num,$goods_id); //获得运费，会员id，地址id，商品数量，商品id
    }
    ...
    public function getRefundMaxMoney($order_goods_id, $num) {
        ...
        $order_goods = $this->where(array('id' => $order_goods_id))->find();
        if (!$order_goods) {
            return false;
        }
        //var_dump($order_goods_id);
        $order=D('Integralorder')->where(array('order_id'=>$order_goods_id))->find();
        //用户实际支付金额 = 订单商品总价 + 配送费 - 手机立减 - 积分抵扣 - 优惠券抵扣
        //$actual_pay = $order_goods['total_price'] + $order_goods['express_price'] - $order_goods['mobile_fan'] - $order_goods['use_integral'] - $order_goods['coupon_price'];
        //修改，用户申请退款只能退商品钱
        $actual_pay = $order['need_pay']-$order['express_price'];
        //var_dump($actual_pay);die;
        return round(($actual_pay/$order_goods['num']) * $num, 2);
    }
