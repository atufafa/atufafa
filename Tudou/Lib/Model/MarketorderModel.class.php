...
class MarketorderModel extends CommonModel {
	...
	public function cancel($order_id,$user_id){
		if($detail = $this->find($order_id)){
			...
			if($Shop['is_market_pei'] == 1){
				...
				if($do){
					if($do['status'] == 2 || $do['status'] == 10 || $do['status'] == 11) {
			...
			if($detail['status'] != 2){
				...
				if (D('Marketorder')->save(array('order_id' => $order_id, 'status' => 8,'end_time' => NOW_TIME))) { //防止并发请求
					...
					if($detail['settlement_price'] > 0) {
						...
						D('Users')->AddUser_guide($detail['user_id'],$detail['settlement_price'],$order_id,$type='market');
	...
	public function AddDeliveryIogistics($order_id){
		if($detail = D('Marketorder')->find($order_id)){
			$Market = D('Market')->find($detail['shop_id']);
        	$Shop = D('Shop')->find($detail['shop_id']);
            $do = D('DeliveryOrder')->where(array('type_order_id' => $order_id, 'type' => 3))->find();//0是商城1是配送
			if($Shop['is_market_pei'] == 1){
                if ($do['logistics_price'] > 0) {
                    ...
                    if (!empty($deliver['recommend']) && !empty($deliveradmin['user_id'])) {
                        ...
                        $data1 = array('d_id' => $deliver['id'], 'sq_id' => $deliveradmin['sq_id'], 'order_id' => $order_id, 'order_type' => 3, 'money' => $fen1,
                            'time' => date('Y-m-d H:i:s'), 'explain' => '菜市场获得分成');
                        ...
                        if (!empty($deliveradmin2['user_id']) && $deliveradmin['dj_id'] > $deliveradmin2['dj_id']) {
                            ...
                            $data2 = array('d_id' => $deliver['id'], 'sq_id' => $deliveradmin2['sq_id'], 'order_id' => $order_id, 'order_type' => 3, 'money' => $fen2,
                                'time' => date('Y-m-d H:i:s'), 'explain' => '菜市场获得分成');
                            ...
                            if (!empty($deliveradmin3['user_id']) && $deliveradmin2['dj_id'] > $deliveradmin3['dj_id']) {
                                ...
                                $data3 = array('d_id' => $deliver['id'], 'sq_id' => $deliveradmin3['sq_id'], 'order_id' => $order_id, 'order_type' => 3, 'money' => $fen3,
                                    'time' => date('Y-m-d H:i:s'), 'explain' => '菜市场获得分成');
                    ...
                    D('Runningmoney')->add_delivery_logistics($order_id, $logistics,3,$deliveradminpay);//配送费接口
                    return true;
                }
			}else{
				return true;
   ...
   public function combination_market_print($order_id,$addr_id) {	
	...
	public function updUseEnvelope($order_id)
	{
		...
		D('UserEnvelopeLogs')->add([
			...
			'intro' =>'在菜市场购买商品使用，订单号为'.$order['order_id'],
   ...
   public function market_delivery_order($order_id,$wait = 0) {	
           ...
           //查询专送配送差价
            $chajia=$config['freight']['market_chajia'];//最低公里差价
            $zdgls=$config['freight']['market_distance'];//最低公里数
            $jiachajia=$config['freight']['market_jia'];//每超过1公里，加扣差价

           //查询直达配送差价
           $zdchajia=$config['freight']['zdmarket_chajia'];//最低公里差价
           $zdzdgls=$config['freight']['zdmarket_distance'];//最低公里数
           $zdjiachajia=$config['freight']['zdmarket_jia'];//每超过1公里，加扣差价
			...
			if($order['logistics'] > 0){
				...
				$logistics_price = ($market['logistics'] > 10) ? $market['logistics'] : $shops['express_price'];
			}
            ...
            D('Marketorder')->where(['order_id' => $order_id])->save(array('status' => 9));
	...
	public function get_logistics($total_money,$shop_id){	
	   $Market = D('Market')->where(array('shop_id' => $shop_id) )->find();
	   if($Market['logistics_full'] <= $total_money){
