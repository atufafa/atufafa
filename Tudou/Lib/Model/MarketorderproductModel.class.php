<?php
class MarketorderproductModel extends CommonModel
{
    protected $pk = 'id';
    protected $tableName = 'market_order_product';
    public function updateByOrderId($order_id)
    {
        ...
        foreach ($datas as $val) {
            D('Marketproduct')->save(array('product_id' => $val['product_id'], 'month_num' => $val['num']));
        }
        foreach ($datas2 as $val) {
            D('Marketproduct')->save(array('product_id' => $val['product_id'], 'sold_num' => $val['num']));
        }
        return true;
    }
}