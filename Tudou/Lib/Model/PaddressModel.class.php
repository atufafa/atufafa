...
//单个付款获取用户地址==会员兑换
    public function order_address_exp($uid,$order_id){
        $obj = D('Paddress');
        if (empty($order_id)) {
            return false;
        } else {
            $Order = D('ExchangeOrder') -> where(array('order_id' =>$order_id))-> find();
            $defaultAddress = $obj -> where(array('id' =>$Order['address_id'],'closed' => 0))-> find();
            if (!empty($defaultAddress)) {
                return $defaultAddress;
            } else {
    ...
    //单个付款获取用户地址
    public function order_address_ids($uid,$order_id){
        $obj = D('Paddress');
        if (empty($order_id)) {
            return false;
        } else {
            $Order = D('Integralorder') -> where(array('order_id' =>$order_id))-> find();
            $defaultAddress = $obj -> where(array('id' =>$Order['address_id'],'closed' => 0))-> find();
            if (!empty($defaultAddress)) {
                return $defaultAddress;
            } else {
    ...
    //单个付款获取用户地址
    public function order_address($uid,$order_id){
        $obj = D('Paddress');
        if (empty($order_id)) {
            return false;
        } else {
            $Order = D('Tuanorder') -> where(array('order_id' =>$order_id))-> find();
            $defaultAddress = $obj -> where(array('id' =>$Order['addr_id'],'closed' => 0,'user_id'=>$uid))-> find();
            if (!empty($defaultAddress)) {
                return $defaultAddress;
            } else {
                return false;
	...
	public function pc_pay_address($address_id,$uid){
        if (!empty($address_id)) {
            ...
            $addrs = D('Paddress')->where(array('user_id' => $uid))->order('id DESC')->limit(0, 6)->select();
            if (empty($addrs)) {
	...
	public function defaultAddress($uid,$type){
			...
			if ($addressCount == 0) {
				$this->tuMsg(U('wap/address/addrcat', array('type' => goods, 'order_id' => $order_id)));//应该有问题
			} else {
