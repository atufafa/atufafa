<?php
class PindanorderModel extends CommonModel{
    protected $pk = 'order_id';
    protected $tableName = 'pindan_order';
	...
	public function update_guoqi_time($guoqi_time){
		...
		$itmes = D('Pindanorder')->where(array('create_time'=>array('lt',$max_time),'status'=>'0'))->select();
		$array = $orders = array();
		...
		if(D('Pindanorder')->where(array('order_id'=>array('in',$order_list)))->save(array('status'=>'-1','update_time'=>$time))){
			foreach($array as $k => $v){
				D('Pindan')->where(array('tuan_id'=>$k))->setInc('num',$v);
				D('Pindan')->where(array('tuan_id'=>$k))->setDec('sold_num',$v);
			}
		}
	}
	//获取抢购实际价格
	public function get_tuan_need_pay($order_id,$user_id,$type){
        ...
        if (empty($order) || $order['status'] != 0 || $order['user_id'] != $user_id) {
			...
			D('Pindanorder')->save(array('order_id' => $order_id, 'use_integral'=>$order['use_integral'],'need_pay' => $order['need_pay']));
        ...
        if($detail = $this->find($order_id)){
            if($detail['status'] != 2 && $detail['status'] != 3) {
                ...
                if($this->save(array('status' => 8, 'order_id' => $order_id))){
                    D('Pindanorder')->save(array('status' => 8), array('where' => array('order_id' => $order_id)));//先更新
                    ...
                    if($detail['is_daofu'] == 0){
                        ...
                        D('Users')->return_integral($Shop['user_id'], $detail['use_integral'] , '0元购物返还给商家');//商城用户积分兑换返还给商家
                        D('Users')->getProit($detail['user_id'],'goods',$settlement_price,$detail['shop_id'],$order_id); //新增商品分销结算
                        if($config['prestige']['is_goods']){
                            D('Users')->reward_prestige($detail['user_id'], (int)($settlement_price),'0元购物返'.$config['prestige']['name']);//返威望
                        }
                    }
    ...
    //更换地址
    public function update_addr($uid,$type,$order_id,$address_id){
        $order = D('Pindanorder')->where(['user_id'=>$uid,'order_id'=>$order_id])->save(['addr_id'=>$address_id]);
        if(false ==$order){
            return false;
        }
        return true;
