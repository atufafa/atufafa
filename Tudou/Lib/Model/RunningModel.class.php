...
class RunningModel extends CommonModel{
	...
	public function Running_Confirm_Complete($running_id,$cid){
		$running = D('Running')->find($running_id);
		$delivery = D('Delivery')->where(array('id'=>$running['cid']))->find();
        $config = D('Setting')->fetchAll();
        $sum_need=$running['need_pay']-$config['running']['chajia'];
        $sum_pei=$running['freight']-$config['running']['chajia'];
		if(!empty($running)){
			if (false !== D('Running')->save( array('running_id' => $running_id,'status' => 3,'end_time' => NOW_TIME))){
                    $info = '跑腿订单ID【'.$running_id.'】结算，实际支付：【'.round($sum_need,2).'元】';
                    $infos = '跑腿订单ID【'.$running_id.'】结算，实际支付：【'.round($sum_need,2).'元】,其中购买物品费用为：【'.round($running['price'],2).'】元';
					if($running['need_pay'] > 0) {
						...
						D('Users')->addMoney($delivery['user_id'], $running['dashan']+$sum_pei,$info);
						D('Users')->addMoneys($delivery['user_id'], $running['price'],$infos);   //写入配送员余额
                    }
					D('Sms')->sms_Running_Delivery_User($running_id);//给买家通知配送状态
                    return true;
		 		}
