...
class SellerAction extends CommonAction
{
    ...
    public function zans()
    {
        ...
        if (empty($detail)) {
            $this->tuError('您点赞的内容不存在');
        }
        D('Shopcomment')->updateCount($comment_id, 'zan');
        $this->tuSuccess('恭喜您，点赞成功', U('seller/detail', array('news_id' => $detail['post_id'])));
    }
    public function post()
    {
        ...
        if (empty($data['content'])) {
            $this->tuError('评论内容不能为空');
        }
        if (empty($data['post_id'])) {
            $this->tuError('文章编号不正确');
        }
        ...
        if (D('Shopcomment')->add($data)) {
            $this->tuSuccess('回复成功', U('seller/detail', array('news_id' => $data['post_id'])));
        }
    }
    /**
     *递归获取评论列表
