...
class ShopModel extends CommonModel{
	...
	//竞价写入
    // public function set_bidmoney($shop_id, $num, $intro = '',$check_price){
    //     print_r($shop_id);die;
    //     if ($this->updateCount($user_id, 'bid_money', $num)) {
    //         D('Weixinmsg')->weixinTmplCapital($type = 1,$user_id,$num,$intro);//余额模板通知
    //         return D('ShopbidlogsModel')->add(array(
    //             'shop_id' => $shop_id, 
    //             'money' => $num, 
    //             'intro' => $intro, 
    ...
    //竞价充值回调
    public function save_jingjia($log_id)
    {
        ...
        if (!empty($shop)) {
            ...
            $shop['bid_money'] +=$detail['need_pay'];
                D('Shop')->where(array('shop_id'=>$shop['shop_id']))->save(['bid_money'=>$shop['bid_money']]);
                return TRUE; 
            // }
        }else{
            return TRUE; 
