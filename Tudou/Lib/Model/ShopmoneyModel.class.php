    ...
    public function insertData($order_id,$id, $shop_id, $jiesuan_price, $type, $info){
     ...
     //    die;
		...
		if($jiesuan_price > 0){
			...
			// 	}
			// }
//            var_dump($jiesuan_price,$profit);
			$jiesuan_price = $jiesuan_price  - $profit;
//			var_dump($jiesuan_price);die;
			// print_r($jiesuan_price);die;
			$intro .='结算价格【'.round($jiesuan_price,2).'元】';
			if($profit){
				$intro .='-扣除分销金额：【'.round($profit,2).'元】';
			}
