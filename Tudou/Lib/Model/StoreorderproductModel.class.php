<?php
class StoreorderproductModel extends CommonModel
{
    protected $pk = 'id';
    protected $tableName = 'store_order_product';
    public function updateByOrderId($order_id)
    {
        ...
        foreach ($datas as $val) {
            D('Storeproduct')->save(array('product_id' => $val['product_id'], 'month_num' => $val['num']));
        }
        foreach ($datas2 as $val) {
            D('Storeproduct')->save(array('product_id' => $val['product_id'], 'sold_num' => $val['num']));
        }
        return true;
    }
}