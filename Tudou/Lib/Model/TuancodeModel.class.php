...
class TuancodeModel extends CommonModel{
	...
	public function saveShopMoney($data,$shop){
			...
			if($data['real_integral'] > 0){
				...
				$intro = '抢购订单号【'.$data['order_id'].'】，抢购卷ID【'.$data['code_id'].'】结算价格 = 结算价【'.round($data['settlement_price'],2).'元】 - 积分抵扣金额【'.round($data['real_integral'],2).'元】';
			}else{
				$settlement_price = $data['settlement_price'];
				$intro = '抢购订单号【'.$data['order_id'].'】，抢购卷ID【'.$data['code_id'].'】结算价格 = 结算价【'.round($data['settlement_price'],2).'元】';
			}
