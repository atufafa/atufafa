...
//    //代理分销
//    public function agent_distribution($order_id, $id, $shop_id, $jiesuan_price, $type)
//    {
//        return true;
//        //获取商品分销总比例
//        $config = D('Setting')->fetchAll();
...
//        if ($money > 0) {
//            if ($Users["fuid1"] > 0) {
//                $agent = D('UsersAgentApply')->getField('user_id,user_guide_id');
//                $datalist = array();
//                foreach ($agent as $key => $value) {
//                    //判断一级
//                    if ($value["user_id"] == $Users["fuid1"]) {
//                        $datalist[$value["user_id"]] = $value["agent_id"];
//                        //判断有没有退款，时间有没有到发放时间
//                        $is_end = $this->is_refund($order_id);
//
//                        //判断是不是还有上级
//                        if ($value["user_guide_id"] > 0) {
//                            $datalist[$value["user_guide_id"]] == $agent[$value["user_guide_id"]]["agent_id"];
//                            $three = $agent[$value["user_guide_id"]]["user_guide_id"];
//                            //判断是不是存在上级
//                            if ($three > 0) {
//                                $datalist[$agent[$three]["user_id"]] == $agent[$three]["agent_id"];
//                                $si = $agent[$three["user_guide_id"]]["user_guide_id"];
//                                //判断是不是存在上级
//                                if ($si > 0) {
//                                    $datalist[$agent[$si]["user_id"]] == $agent[$si]["agent_id"];
//                                }
//                            }
//                        }
//                    }
...
//                    //判断代理等级
//                    $fenxiao = D("Cityagent")->where(array("agent_id" => $value))->field("rate")->find();
//                    $price = $money * $fenxiao["rate"] / 100;
//                    $data[$key] = $sum - $price;
//                    $count = $count + ($sum - $price);
//                }
//
//                $data[$Users] = $money - $count;
//                var_dump($data);
//                die;
//
//            } else {
//                $data = D("Users")->where(array("user_id" => $Users))->setInc('money', $Users);
//                if ($data) {
//                    return false;
//                }
//                return true;
...
//    }
        ...
        if ($money > 0) {
            ...
            if (!empty($tui['is_backers']) && $tui['is_backers'] == 2) {
                if ($is_end == true) {
                    ...
                    $money1 = $one3['reward'] * $money;
                    $info1 = '推手分成金额' . $money1 . '元，推手分成【' . $one3['reward'].'】%';
                ...
                if ($is_end == true) {
                    ...
                    $money1 = $city_agents['rate'] * $money;
                    $info1 = '代理分成金额' . $money1 . '元，代理分成【 ' . $city_agents['reward'].'】%';
            ...
            if (!empty($RebateData1['parent_id'])) {
                ...
                if ($is_end == true) {
                    ...
                    $money2 = $RebateData1['rate'] * $money;
                    $info1 = '城市合伙人分成金额' . $money2 . '元，分成比例【' . $RebateData1['rate'] . '】%';

                    $log['order_id'] = $order_id;
                    $log['user_id'] = $RebateData1['parent_id'];
                    $log['total_money'] = $money;
                    $log['rate'] = $sum;
                    ...
                    $log['remark'] = $info1;
                    self::rebate_log($log);
                    D('Users')->addMoney($RebateData1['parent_id'], $money2, $info1);
                }
                ...
                if (!empty($RebateData2['parent_id']) && $RebateData1['rate_level'] < $RebateData2['rate_level']) {
                    ...
                    if ($is_end == true) {
                        ...
                        $money3 = $RebateData2['rate'] * $money;
                        $info1 = '县级分成金额' . $money3 . '元，分成比例【' . $RebateData2['rate'] . '】%';

                        $log['order_id'] = $order_id;
                        $log['user_id'] = $RebateData2['parent_id'];
                        $log['total_money'] = $money;
                        $log['rate'] = $sum;
                        ...
                        $log['remark'] = $info1;
                        self::rebate_log($log);
                        D('Users')->addMoney($RebateData2['parent_id'], $money3, $info1);
                    }

                    //判断有上一级==市级
                    $RebateData3 = D('UsersAgentApply')->RebateData($RebateData2['parent_id']);
                    if (!empty($RebateData3['parent_id']) && $RebateData2['rate_level'] < $RebateData3['rate_level']) {
                        //判断是否有退款或到时间发放
                        $is_end = $this->is_refund($order_id);
                        if ($is_end == true) {
                            $platform_rate -= $RebateData3['rate'];
                            $money4 = $RebateData3['rate'] * $money;
                            $info1 = '市级分成金额' . $money4 . '元，分成比例【' . $RebateData3['rate'] . '】%';

                            $log['order_id'] = $order_id;
                            $log['user_id'] = $RebateData3['parent_id'];
                            $log['total_money'] = $money;
                            $log['rate'] = $sum;
                            ...
                            $log['remark'] = $info1;
                            self::rebate_log($log);
                            D('Users')->addMoney($RebateData3['parent_id'], $money4, $info1);
                        }
                    }
                }
            }
            $money5 = $platform_rate * $money;
    ...
    public function profitUsers($order_id, $id, $shop_id, $jiesuan_price, $type)
    {
        ...
        if ($Shop['is_profit']) {
            list($user_id, $money) = $this->getModelMoneyUser($order_id, $id, $jiesuan_price, $type);
            var_dump('到底是多少钱：'.$money,'是不是自己：'.$user_id);die;
            $obj = D('Users');
            $Users = $obj->find($user_id);
            ...
            if ($money > 0) {
                if ($Users['fuid1'] && (true == $this->determinePower($Users['fuid1']))) {
                    ...
                    if ($type == 'goods' && $goods_profit['profit_enable'] == 1 && ($goods_profit['profit_rate1']) > 1) {
                        ...
                        $info1 = $this->_type[$type] . '商城订单ID:' . $order_id . ', 一级分成: ' . round($money1 , 2) . '商品独立分成比例【' . $rate1 . '】%，分成类型【' . $this->_profit_price_type[$config['profit']['profit_price_type']] . '】';
                    } elseif ($type == 'goods') {
                        $money1 = round($config['profit']['goods_profit_rate1'] * $money /100,2 );
                        $rate1 = $config['profit']['goods_profit_rate1'];
                        $info1 = $this->_type[$type] . '商城订单ID:' . $order_id . ', 一级分成: ' . round($money1 , 2) . '分成比例【' . $rate1 . '】%，分成类型【' . $this->_profit_price_type[$config['profit']['profit_price_type']] . '】';
                    } elseif ($type == 'hotel') {
                        $money1 = round($config['profit']['hotel_profit_rate1'] * $money /100,2 );
                        $rate1 = $config['profit']['hotel_profit_rate1'];
                        $info1 = $this->_type[$type] . '酒店订单ID:' . $order_id . ', 一级分成: ' . round($money1 , 2) . '分成比例【' . $rate1 . '】%，分成类型【' . $this->_profit_price_type[$config['profit']['profit_price_type']] . '】';
                    } elseif ($type == 'farm') {
                        $money1 = round($config['profit']['farm_profit_rate1'] * $money  /100,2);
                        $rate1 = $config['profit']['farm_profit_rate1'];
                        $info1 = $this->_type[$type] . '农家乐订单ID:' . $order_id . ', 一级分成: ' . round($money1 , 2) . '分成比例【' . $rate1 . '】%，分成类型【' . $this->_profit_price_type[$config['profit']['profit_price_type']] . '】';
                    } elseif ($type == 'breaks') {
                        $money1 = round($config['profit']['breaks_profit_rate1'] * $money /100,2);
                        $rate1 = $config['profit']['breaks_profit_rate1'];
                        $info1 = $this->_type[$type] . '优惠买单订单ID:' . $order_id . ', 一级分成: ' . round($money1 , 2) . '分成比例【' . $rate1 . '】%，分成类型【' . $this->_profit_price_type[$config['profit']['profit_price_type']] . '】';
                    } else {
                        $money1 = round($config['profit']['currency_profit_rate1'] * $money /100,2);
                        $rate1 = $config['profit']['currency_profit_rate1'];
                        $info1 = $this->_type[$type] . '订单ID:' . $order_id . ', 一级分成: ' . round($money1 , 2) . '分成比例【' . $rate1 . '】%，分成类型【' . $this->_profit_price_type[$config['profit']['profit_price_type']] . '】';
                    }
                ...
                if ($Users['fuid2'] && (true == $this->determinePower($Users['fuid2']))) {
                    ...
                    if ($type == 'goods' && $goods_profit['profit_enable'] == 1 && ($goods_profit['profit_rate2']) > 1) {
                        ...
                        $info2 = $this->_type[$type] . '订单ID:' . $order_id . ', 二级分成: ' . round($money2, 2) . '商品独立分成比例【' . $rate2 . '】%，分成类型【' . $this->_profit_price_type[$config['profit']['profit_price_type']] . '】';
                    } elseif ($type == 'goods') {
                        $money2 = round($config['profit']['goods_profit_rate2'] * $money /100,2);
                        $rate2 = $config['profit']['goods_profit_rate2'];
                        $info2 = $this->_type[$type] . '订单ID:' . $order_id . ', 二级分成: ' . round($money2, 2) . '分成比例【' . $rate2 . '】%，分成类型【' . $this->_profit_price_type[$config['profit']['profit_price_type']] . '】';
                    } elseif ($type == 'hotel') {
                        ...
                        $info2 = $this->_type[$type] . '优惠买单订单ID:' . $order_id . ', 二级分成: ' . round($money2 , 2) . '分成比例【' . $rate2 . '】%，分成类型【' . $this->_profit_price_type[$config['profit']['profit_price_type']] . '】';
                    } else {
