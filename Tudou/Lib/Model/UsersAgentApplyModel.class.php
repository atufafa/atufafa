...
class UsersAgentApplyModel extends CommonModel {
    ...
    public function  RebateData($user_id){
        $UsersAgentApply=$this->where(array('user_id'=>$user_id))->find();
        $parent_id=isset($UsersAgentApply['user_guide_id'])?$UsersAgentApply['user_guide_id']:0;
        $agent_id=isset($UsersAgentApply['agent_id'])?$UsersAgentApply['agent_id']:0;
        $agent_is=isset($UsersAgentApply['user_id'])?$UsersAgentApply['user_id']:0;
        $RebateData=M('city_agents')->where(array('agent_id'=>$agent_id))->find();
        $RebateData['parent_id']=$parent_id;
        $RebateData['user_id']=$agent_is;
        return $RebateData;
    }
}
