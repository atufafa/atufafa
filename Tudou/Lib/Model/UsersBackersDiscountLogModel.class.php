	...
	public function UsersBackersDiscount($order_id,$type){
		if($type == 'goods'){
			...
			if($Shop['is_goods_backers']){
				$money = $Order['need_pay'] - (($Order['need_pay']*$Userrank['discount']));
			}else{
				$money = 0;
			}
			...
			if($Shop['is_ele_backers']){
				$money = $Eleorder['need_pay'] - (($Eleorder['need_pay']*$Userrank['discount']));
			}else{
				$money = 0;
			}
			...
			$Userrank = D('Userrank')->where(array('rank_id'=>$Users['rank_id']))->find();
			if($Shop['is_ele_backers']){
				$money = $Marketorder['need_pay'] - (($Marketorder['need_pay']*$Userrank['discount']));
			}else{
				$money = 0;
			}
			...
			$Userrank = D('Userrank')->where(array('rank_id'=>$Users['rank_id']))->find();
			if($Shop['is_ele_backers']){
				$money = $Storeorder['need_pay'] - (($Storeorder['need_pay']*$Userrank['discount']));
			}else{
				$money = 0;
			}
