...
class UsersBackersRewardLogModel extends CommonModel{
	...
	public function UsersBackersReward($order_id,$type){
		...
		if($type == 'goods'){
			...
			if($Shop['is_goods_backers']){
				//计算分成金额
				$money = (int)(($Order['need_pay']*$Userrank['reward']));
			}else{
				$money = 0;
			}
			...
			if($Shop['is_ele_backers']){
				$money = (int)(($Eleorder['need_pay']*$Userrank['reward']));
			}else{
				$money = 0;
			}
			...
			$Userrank = D('Userrank')->where(array('rank_id'=>$Users['rank_id']))->find();
			$money = (int)(($Marketorder['need_pay']*$Userrank['reward']));
			...
			$Userrank = D('Userrank')->where(array('rank_id'=>$Users['rank_id']))->find();
			$money = (int)(($Storeorder['need_pay']*$Userrank['reward']));
			$intro = '【推手】便利店订单ID：【'.$order_id.'】下级购买';
		}
		
		list($fuid,$type)= $this->getFuid($user_id);
		list($fuid1,$type)= $this->getFuidA($fuid);
		if($money > 0){
			$data = array();
			...
			$data['create_ip'] = get_client_ip();
			$this->add($data);
			D('Users')->addMoney($fuid1,$money,$intro.''.$type);
			return $money;
		}
		return 0;
	}
	...
	//循环下级获取分成，第一推荐人，第二推荐人，第三推荐人
	public function getFuidA($user_id){
		$obj = D('Users');
		$Users = $obj->find($user_id);
		if($Users['fuid1']){
			$fuid = $obj->find($Users['fuid1']);
			if($fuid['is_backers'] == 2){
				return array($Users['fuid1'],$type = '【二级分成】');
			}
		}
		return true;
	}
