<?php
class VehiclecashModel extends CommonModel{
    protected $pk   = 'cash_id';
    protected $tableName =  'vehicle_cash';
	...
	public function alipayUserCach($cash_id,$tpye){
		...
		$money = ($detail['type'] =='user') ? $detail['errandsmoney']: $detail['gold'];//获取金额
	
		if($money < 10){
			$this->error = '申请提现的金额不合法';
		    return false;
		}
		$money = round($money,2);//金额优化
	...
	public function weixinUserCach($cash_id,$tpye){
		...
		$money = ($detail['type'] =='user') ? $detail['money']: $detail['gold'];//获取金额
