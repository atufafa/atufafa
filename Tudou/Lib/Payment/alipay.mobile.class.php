...
class alipay {
    ...
    public function getCode($logs, $setting){
        ...
        $para_token = array(
            ...
            'v' => '2.0',
            'req_id' => $req_id,
            'req_data' =>'<direct_trade_create_req><notify_url>'. $config['site']['host'] . U('Wap/payment/respond', array('code' => 'alipay')) .'</notify_url><call_back_url>' . $config['site']['host'] . U('Wap/payment/respond', array('code' => 'alipay')) .'</call_back_url><seller_account_name>' .$setting['alipay_account'] .'</seller_account_name><out_trade_no>' . $out_trade_no . '</out_trade_no><subject>'. $logs['subject'] .'</subject><total_fee>'. $logs['logs_amount'] .'</total_fee><merchant_url>' .$config['site']['host'] . U('index/index') .'</merchant_url></direct_trade_create_req>'
        );
