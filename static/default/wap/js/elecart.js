    ...
    summoney:function(shop_id){
        ...
        if (!goods) {
            ...
            for (var i in goods) {
                if(i==shop_id){
                    for (var index in goods[i]) {
                        num += (goods[i][index]['cost_price'] - goods[i][index]['price'])*goods[i][index]['num'];
                        if(num>0){
                ...
                for (var sid in goods) {
                    ...
                    if (sid == shop_id) {
                        ...
                        for (var index in goods[sid]) {
                            ...
                            console.log(goods);
                            if (goods[sid][index]['product_id'] == datas['product_id']) {
								//goods[sid][index]['num'] = datas['num'];
								//console.log(goods[sid][index]['num']);
                                is_here = true;
                                break;
                            }
                        }
                ...
                if (is_in) {
                    ...
                    if (is_here) {
                        ...
                        if (window.market.count() < 99) {
                            goods[shop_id][index]['num']= datas['num'];
                            //layer.msg('购物车商品数已经满99,不能再添加商品');
                        }
...
window.store = {
            ...
            if (!cookies.isset('store')) {
                ...
                for (var sid in goods) {
                    if (sid == shop_id) {
                        ...
                        is_in = true;
                        for (var index in goods[sid]) {
                            //goods[sid][index]['product_id'] == datas['product_id'] && goods[sid][index]['spec']==datas['spec']
                            if (goods[sid][index]['product_id'] == datas['product_id'] && goods[sid][index]['spec']==datas['spec']) {
                ...
                if (is_in) {
                    //商品存在
                    if (is_here) {
                        if (window.store.count() < 99999999) {
                            if(goods[shop_id][index]['spec']==datas['spec'] && goods[shop_id][index]['product_id']==datas['product_id']){
                                goods[shop_id][index]['num']++;
                            }
    ...
    dec: function (shop_id, product_id,spec) {
        var goods = window.store.getcart();
        if (!goods) {
            //这种情况暂时不会发生
            layer.msg('该商品不在购物车中,请重新添加');
            ...
            //假设该商品存在
            for (var i in goods[shop_id]) {
                if (goods[shop_id][i]['product_id'] == product_id && goods[shop_id][i]['spec']== spec) {
                    if (window.store.itemcount(product_id,spec) <= 0) {
                        return false;
                    } else {
                        goods[shop_id][i]['num']--;
                        goods = window.cookies.stringify(goods);
    ...
    itemcount: function (product_id,spec) {
        var goods = window.store.getcart();
        if (!goods) {
            return '0';
        } else {
            ...
            for (var i in goods) {
                for (var index in goods[i]) {
                    if (goods[i][index]['product_id'] == product_id && goods[i][index]['spec']==spec) {
                        num = goods[i][index]['num'];
                    }
                }
            }
    ...
    removeby: function (product_id,spec) {
